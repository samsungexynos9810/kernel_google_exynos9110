...
static inline int sysmmu_set_prefetch_buffer_by_plane(struct device *dev,
			unsigned int inplanes, unsigned int onplanes,
			unsigned int ipoption, unsigned int opoption)
{
	return -ENOSYS;
...
static inline int exynos_sysmmu_map_user_pages(struct device *dev,
					struct mm_struct *mm,
					unsigned long vaddr, size_t size,
					int write)
{
...
static inline int exynos_sysmmu_unmap_user_pages(struct device *dev,
					struct mm_struct *mm,
					unsigned long vaddr, size_t size)
{
	return -ENODEV;
...
static inline int exynos_sysmmu_set_ppc_event(struct device *dev, int event)
{
	return -ENOSYS;
}
