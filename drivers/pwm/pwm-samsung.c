...
static int s3c_pwm_enable(struct pwm_chip *chip, struct pwm_device *pwm)
{
	...
	if (soc_is_s3c24xx()) {
		...
		tcon = __raw_readl(S3C2410_TCON);
		if (!(tcon & pwm_tcon_start(s3c))) {
			tcon |= pwm_tcon_manulupdate(s3c);
			__raw_writel(tcon, S3C2410_TCON);

			tcon &= ~pwm_tcon_manulupdate(s3c);
			if (s3c->duty_cycle == DUTY_CYCLE_ZERO)
				tcon &= ~pwm_tcon_autoreload(s3c);
			else
				tcon |= pwm_tcon_autoreload(s3c);
			tcon |= pwm_tcon_start(s3c);
			__raw_writel(tcon, S3C2410_TCON);
		} else if (!(tcon & pwm_tcon_autoreload(s3c)) &&
			   s3c->duty_cycle != DUTY_CYCLE_ZERO) {
			tcon |= pwm_tcon_manulupdate(s3c);
			__raw_writel(tcon, S3C2410_TCON);
...
static void s3c_pwm_init(struct s3c_chip *s3c)
{
	unsigned long tcon;
	...
	if (soc_is_s3c24xx()) {
		...
		__raw_writel(0, S3C2410_TCMPB(s3c->pwm_id));
		__raw_writel(0, S3C2410_TCNTB(s3c->pwm_id));
