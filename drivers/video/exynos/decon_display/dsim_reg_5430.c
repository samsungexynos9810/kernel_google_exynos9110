...
void dsim_reg_set_dphy_timing_values(struct dphy_timing_value *t)
{
	...
	dsim_write_mask(DSIM_PHYCTRL, val, DSIM_PHYCTRL_B_DPHYCTL0_MASK);
}
...
static u32 dsim_reg_is_lane_stop_state(void)
{
...
u32 dsim_reg_wait_lane_stop_state(void)
{
	u32 state;
	u32 cnt = 100;

	do {
		state = dsim_reg_is_lane_stop_state();
		cnt--;
	} while (!state && cnt);
...
static u32 dsim_reg_is_ulps_state(void)
{
	u32 val = dsim_read(DSIM_STATUS);
...
int dsim_reg_wait_enter_ulps_state(void)
{
	u32 state;
	u32 cnt = 1000;

	do {
		state = dsim_reg_is_ulps_state();
		cnt--;
		udelay(10);
	} while (!state && cnt);
...
int dsim_reg_set_ulps(u32 en)
{
	...
	if (en) {
		...
		ret = dsim_reg_wait_enter_ulps_state();
		if (ret)
			return ret;
