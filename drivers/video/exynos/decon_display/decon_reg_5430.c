...
/*
 * initialize DECON SFR
 *	- sw reset
 *	- enable clock gate mode
 *	- set 8bit alpha value
 *	- set DECON output interface(RGB or I80)
 *	- set CRC
 *	- set VCLK hold scheme at underrun.
 *	  It depends on AP and mode(video or command)
 *	- clear SFR related to window information
 *	- set information related to LCD
 *	- set trigger mode in case of command mode
 */
...
/*
 * start DECON hardware
 *	- unmask hw trigger in case of using hw trigger
 *	- start DECON hardware
 */
...
/*
 * stop DECON hardware
 *	- mask hw trigger in case of using hw trigger
 *	- wait until linecnt is zero.
 *	- stop DECON hardware
 *	  command mode : direct off, video mode : per frame off
 *	- wait until DECON becomes stop state
 *	- sw reset
 */
...
/* set all information of each window to related SFR */
