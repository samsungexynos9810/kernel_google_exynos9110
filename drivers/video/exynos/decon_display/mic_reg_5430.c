...
void mic_reg_set_mic_base_operation(struct decon_lcd *lcd, bool enable)
{
	...
	if (enable) {
		...
		if (lcd->mic_ver == MIC_VER_1_1)
			data |= MIC_OP_OLD_CORE;
		else /* default mic version 1.2 */
			data |= MIC_OP_NEW_CORE;
...
{
	u32 bs_2d = (lcd->xres / 2) + (lcd->xres % 4);

	mic_reg_sw_reset();
	mic_reg_set_porch_timing(lcd);
	mic_reg_set_image_size(lcd);
	mic_reg_set_output_timing(lcd, bs_2d);
