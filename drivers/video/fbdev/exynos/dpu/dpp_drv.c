...
static void dpp_dump_registers(struct dpp_device *dpp)
{
	...
	print_hex_dump(KERN_INFO, "", DUMP_PREFIX_ADDRESS, 32, 4,
			dpp->res.regs, 0x90, false);
	print_hex_dump(KERN_INFO, "", DUMP_PREFIX_ADDRESS, 32, 4,
			dpp->res.regs + 0xA00, 0x8, false);
	print_hex_dump(KERN_INFO, "", DUMP_PREFIX_ADDRESS, 32, 4,
			dpp->res.regs + 0xA48, 0x10, false);
	print_hex_dump(KERN_INFO, "", DUMP_PREFIX_ADDRESS, 32, 4,
			dpp->res.regs + 0xB00, 0x100, false);
...
static void dpp_get_params(struct dpp_device *dpp, struct dpp_params_info *p)
{
	...
	p->is_rot = (config->dpp_parm.rot >= DPP_ROT_90) ? true : false;
	...
	if (p->is_rot) {
		p->h_ratio = (p->src.h << 20) / p->dst.w;
		p->v_ratio = (p->src.w << 20) / p->dst.h;
	} else {
...
static int dpp_check_scale_ratio(struct dpp_params_info *p)
{
	...
	if (p->src.w > sc_up_max_w || p->src.h > sc_up_max_h) {
		dpp_err("unsupported max(2x) up scale ratio\n");
		goto err;
	}

	if (p->dst.w < sc_down_min_w || p->dst.h < sc_down_min_h) {
		dpp_err("unsupported min(1/8x) down scale ratio\n");
		goto err;
	}

	return 0;
err:
	dpp_err("src w(%d) h(%d), dst w(%d) h(%d), rot(%d)\n",
			p->src.w, p->src.h, p->dst.w, p->dst.h, p->is_rot);
	return -EINVAL;
}

static int dpp_check_rotation(struct dpp_device *dpp)
{
	struct decon_win_config *config = dpp->config;
	enum dpp_rotate rot = config->dpp_parm.rot;
	if (!is_vgr(dpp) && (rot > DPP_ROT_NORMAL)) {
		dpp_err("dpp-%d can't rotate\n", dpp->id);
		return -EINVAL;
	}
...
static int dpp_check_format(struct dpp_device *dpp, struct dpp_params_info *p)
{
	...
	if ((dpp->id == IDMA_G0 || dpp->id == IDMA_G1) &&
			(p->format >= DECON_PIXEL_FORMAT_NV16)) {
		dpp_err("Unsupported YUV format%d in G%d \n", p->format, dpp->id);
		return -EINVAL;
	}

	if (dpp->id != IDMA_VGF0 && dpp->id != IDMA_VGF1) {
		if (p->is_comp) {
			dpp_err("Unsupported AFBC format decoding in DPP%d\n",
					dpp->id);
			return -EINVAL;
		}

		if (p->is_scale) {
			dpp_err("Unsupported Scaling in DPP%d..(V)G-ch\n", dpp->id);
			return -EINVAL;
		}
	}
	...
	return 0;
#else
	if ((dpp->id == 0 || dpp->id == 1 || dpp->id == 4 || dpp->id == 5) &&
			(p->format >= DECON_PIXEL_FORMAT_NV16)) {
		dpp_err("Unsupported YUV format%d in G%d \n",
				p->format, dpp->id);
		return -EINVAL;
	...
	if (dpp->id != 6 && dpp->id != 7) { /* !VGR case */
		...
		if ((p->is_scale) || (p->is_rot)) {
			dpp_err("Unsupported Scailing in dpp%d, s(%d), r(%d)\n",
					dpp->id, p->is_scale, p->is_rot);
			return -EINVAL;
		}
	}
...
static int dpp_check_limitation(struct dpp_device *dpp, struct dpp_params_info *p)
{
	...
	if (p->is_comp && p->is_rot) {
		dpp_err("rotation is not supported in case of compression");
		return -EINVAL;
	}
...
static irqreturn_t dpp_irq_handler(int irq, void *priv)
{
	...
	if (dpp_irq & DPP_CONFIG_ERROR) {
		dpp_err("dpp%d config error irq occur(0x%x)\n",
				dpp->id, dpp_irq);
		dpp_err("DPP_CFG_ERR_STATE(0x%x)\n",
				dpp_read(dpp->id, DPP_CFG_ERR_STATE));
		dpp_dump_registers(dpp);
		goto irq_end;
	}
...
static irqreturn_t dma_irq_handler(int irq, void *priv)
{
	...
	if (dpp->id == ODMA_WB) {
		...
		if (irqs & IDMA_RECOVERY_START_IRQ) {
			dpp_info("dpp%d recovery start(0x%x)\n", dpp->id, irqs);
			goto irq_end;
		}
...
static int dpp_init_resources(struct dpp_device *dpp, struct platform_device *pdev)
{
	...
	if (!res) {
		dpp_err("failed to get irq resource\n");
		return -ENOENT;
	}
	dpp_info("irq no = %lld\n", res->start);
	...
	if (dpp->id != ODMA_WB) {
		...
		if (!res) {
			dpp_err("failed to get dma irq resource\n");
			return -ENOENT;
		}
...
static int dpp_probe(struct platform_device *pdev)
{
	...
	/* TODO: iommu fault handler will be added in the future */
