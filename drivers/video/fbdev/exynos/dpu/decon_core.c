...
void decon_dump(struct decon_device *decon)
{
	...
	print_hex_dump(KERN_ERR, "", DUMP_PREFIX_ADDRESS, 32, 4,
			decon->res.regs, 0x2AC, false);

	print_hex_dump(KERN_ERR, "", DUMP_PREFIX_ADDRESS, 32, 4,
			decon->res.regs + 0x1000, 0x314, false);

	if (decon->lcd_info->mic_enabled) {
		decon_info("\n=== DECON%d MIC SFR DUMP ===\n", decon->id);
		print_hex_dump(KERN_ERR, "", DUMP_PREFIX_ADDRESS, 32, 4,
				decon->res.regs + 0x1000, 0x20, false);
	} else if (decon->lcd_info->dsc_enabled) {
		decon_info("\n=== DECON%d DSC0 SFR DUMP ===\n", decon->id);
		print_hex_dump(KERN_ERR, "", DUMP_PREFIX_ADDRESS, 32, 4,
				decon->res.regs + 0x2000, 0xA0, false);

		decon_info("\n=== DECON%d DSC1 SFR DUMP ===\n", decon->id);
		print_hex_dump(KERN_ERR, "", DUMP_PREFIX_ADDRESS, 32, 4,
				decon->res.regs + 0x3000, 0xA0, false);
	}

	if (decon->dt.dsi_mode != DSI_MODE_SINGLE) {
		decon_info("\n=== DECON%d DISPIF2 SFR DUMP ===\n", decon->id);
		print_hex_dump(KERN_ERR, "", DUMP_PREFIX_ADDRESS, 32, 4,
				decon->res.regs + 0x5080, 0x80, false);

		decon_info("\n=== DECON%d DISPIF3 SFR DUMP ===\n", decon->id);
		print_hex_dump(KERN_ERR, "", DUMP_PREFIX_ADDRESS, 32, 4,
				decon->res.regs + 0x6080, 0x80, false);
	}
#if 0
	decon_info("\n=== DECON%d SHADOW SFR DUMP ===\n", decon->id);
	print_hex_dump(KERN_ERR, "", DUMP_PREFIX_ADDRESS, 32, 4,
			decon->res.regs + SHADOW_OFFSET, 0x2AC, false);
#endif
		...
		struct decon_reg_data *regs)
{
	...
	if ((is_rotation(config->dpp_parm.rot)) || is_scaling(config))
		return ret;
...
/* TODO: This will be modified for KC */
static int decon_check_limitation(struct decon_device *decon, int idx,
		struct decon_win_config *config)
{
	u32 bpp;

	/* IDMA_G0 channel is dedicated to WIN7 */
	...
	if (config->dst.w * bpp / 8 < 128) {
		decon_err("window wide < 128bytes, width = %u, bpp = %u)\n",
				config->dst.w, bpp);
		return -EINVAL;
	}

	if (!decon_validate_x_alignment(decon, config->dst.x,
				config->dst.w, bpp)) {
		return -EINVAL;
	}
		...
		struct decon_reg_data *regs)
{
	...
	for (i = 0; i < decon->dt.max_win && !ret; i++) {
		...
		case DECON_WIN_STATE_COLOR:
			...
			decon_win_conig_to_regs_param(0, config, win_regs, IDMA_G0, i);
			break;
...
static int decon_acquire_window(struct decon_device *decon, int idx)
{
	...
	if ((decon->dt.out_type == DECON_OUT_DSI) && (idx == decon->dt.dft_win)) {
		...
		struct platform_device *pdev, char *name)
{
	...
	if (decon->dt.out_type == DECON_OUT_DSI) {
		...
		if (ret)
			goto err;
		/* HACK : SW TE not workign for now */
		/*
		if (decon->dt.psr_mode != DECON_VIDEO_MODE) {
...
static int decon_probe(struct platform_device *pdev)
{
	...
	void __iomem *te_sysreg;
	u32 val;

	dev_info(dev, "%s start\n", __func__);

	te_sysreg = ioremap(0x1282100c, 0x4);
	val = readl(te_sysreg);
	val |= 0x7;
	writel(val, te_sysreg);
