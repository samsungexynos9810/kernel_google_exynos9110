...
int dsim_write_data(struct dsim_device *dsim, u32 id, unsigned long d0, u32 d1)
{
	...
	case MIPI_DSI_TURN_ON_PERIPHERAL:
		dsim_reg_set_bta_type(dsim->id, false);
		dsim_reg_wr_tx_header(dsim->id, id, d0, d1);
	...
	case MIPI_DSI_GENERIC_READ_REQUEST_2_PARAM:
		dsim_reg_set_bta_type(dsim->id, true);
		dsim_reg_wr_tx_header(dsim->id, id, d0, d1);
		must_wait = dsim_fifo_empty_needed(dsim, id, d0);
		break;
...
static irqreturn_t dsim_irq_handler(int irq, void *dev_id)
{
	...
	if (int_src & DSIM_INTSRC_UNDER_RUN)
		dsim_dbg("dsim%d underrun irq occurs\n", dsim->id);
...
#if defined(CONFIG_SOC_EXYNOS8895)
	/* HACK : virtual lcd in zebu emulator doesn't need gpio control for lcd */
	return 0;
#endif
...
static int dsim_enable(struct dsim_device *dsim)
{
	...
	/* DPHY power on: PHY driver is not ready */
	...
	/*
	 * DPHY reset will do in PHY control framework
	 */
	udelay(300);
...
static int dsim_probe(struct platform_device *pdev)
{
	...
	/* HACK for PHY power on */
	void __iomem *sysreg_lp_mux;
	void __iomem *pmu_iso;
	u32 lp_mux, iso;

	pmu_iso = ioremap(0x16480710, 0x4);
	iso = readl(pmu_iso);
	iso |= 0x1;
	writel(iso, pmu_iso);

	sysreg_lp_mux = ioremap(0x12821000, 0x4);
	lp_mux = 0;
	lp_mux |= 0x1201;
	writel(lp_mux, sysreg_lp_mux);
