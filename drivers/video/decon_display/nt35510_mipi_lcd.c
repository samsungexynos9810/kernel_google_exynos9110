...
static int ktd283_set_brightness(struct backlight_device *bd)
{
	...
	if (brightness < MIN_BRIGHTNESS ||
		...
		printk(KERN_ALERT "lcd brightness should be %d to %d. now %d\n",
...
void init_lcd(struct mipi_dsim_device *dsim)
{
	...
	if (s5p_mipi_dsi_wr_data(dsim, MIPI_DSI_DCS_LONG_WRITE,
				(unsigned int)initcode_0,
				ARRAY_SIZE(initcode_0)) == -1)
	printk("MIPI DSI failed to write CMD0\n");
	if (s5p_mipi_dsi_wr_data(dsim, MIPI_DSI_DCS_LONG_WRITE,
				(unsigned int)initcode_1,
				ARRAY_SIZE(initcode_1)) == -1)
	printk("MIPI DSI failed to write CMD1\n");
	if (s5p_mipi_dsi_wr_data(dsim, MIPI_DSI_DCS_LONG_WRITE,
				(unsigned int)initcode_2,
				ARRAY_SIZE(initcode_2)) == -1)
	printk("MIPI DSI failed to write CMD2\n");
	if (s5p_mipi_dsi_wr_data(dsim, MIPI_DSI_DCS_LONG_WRITE,
				(unsigned int)initcode_3,
				ARRAY_SIZE(initcode_3)) == -1)
	printk("MIPI DSI failed to write CMD3\n");
	if (s5p_mipi_dsi_wr_data(dsim, MIPI_DSI_DCS_LONG_WRITE,
				(unsigned int)initcode_4,
				ARRAY_SIZE(initcode_4)) == -1)
	printk("MIPI DSI failed to write CMD4\n");
	if (s5p_mipi_dsi_wr_data(dsim, MIPI_DSI_DCS_LONG_WRITE,
				(unsigned int)initcode_5,
				ARRAY_SIZE(initcode_5)) == -1)
	printk("MIPI DSI failed to write CMD5\n");
	if (s5p_mipi_dsi_wr_data(dsim, MIPI_DSI_DCS_LONG_WRITE,
				(unsigned int)initcode_6,
				ARRAY_SIZE(initcode_6)) == -1)
	printk("MIPI DSI failed to write CMD6\n");

	s5p_mipi_dsi_wr_data(dsim, MIPI_DSI_DCS_SHORT_WRITE_PARAM,
				initcode_7[0],initcode_7[1]);

	if (s5p_mipi_dsi_wr_data(dsim, MIPI_DSI_DCS_LONG_WRITE,
				(unsigned int)initcode_8,
				ARRAY_SIZE(initcode_8)) == -1)
	printk("MIPI DSI failed to write CMD8\n");
	if (s5p_mipi_dsi_wr_data(dsim, MIPI_DSI_DCS_LONG_WRITE,
				(unsigned int)initcode_9,
				ARRAY_SIZE(initcode_9)) == -1)
	printk("MIPI DSI failed to write CMD9\n");
	if (s5p_mipi_dsi_wr_data(dsim, MIPI_DSI_DCS_LONG_WRITE,
				(unsigned int)initcode_10,
				ARRAY_SIZE(initcode_10)) == -1)
	printk("MIPI DSI failed to write CMD10\n");
	if (s5p_mipi_dsi_wr_data(dsim, MIPI_DSI_DCS_LONG_WRITE,
				(unsigned int)initcode_11,
				ARRAY_SIZE(initcode_11)) == -1)
	printk("MIPI DSI failed to write CMD11\n");
	msleep(120);

	/* display parameter */
	if (s5p_mipi_dsi_wr_data(dsim, MIPI_DSI_DCS_LONG_WRITE,
				(unsigned int)initcode2_0,
				ARRAY_SIZE(initcode2_0)) == -1)
	printk("MIPI DSI failed to write CMD2_0\n");
	if (s5p_mipi_dsi_wr_data(dsim, MIPI_DSI_DCS_LONG_WRITE,
				(unsigned int)initcode2_1,
				ARRAY_SIZE(initcode2_1)) == -1)
	printk("MIPI DSI failed to write CMD2_1\n");

	s5p_mipi_dsi_wr_data(dsim, MIPI_DSI_DCS_SHORT_WRITE_PARAM,
				initcode2_2[0], initcode2_2[1]);

	s5p_mipi_dsi_wr_data(dsim, MIPI_DSI_DCS_SHORT_WRITE_PARAM,
				initcode2_3[0], initcode2_3[1]);

	if (s5p_mipi_dsi_wr_data(dsim, MIPI_DSI_DCS_LONG_WRITE,
				(unsigned int)initcode2_4,
				ARRAY_SIZE(initcode2_4)) == -1)
	printk("MIPI DSI failed to write CMD2_4\n");
	if (s5p_mipi_dsi_wr_data(dsim, MIPI_DSI_DCS_LONG_WRITE,
				(unsigned int)initcode2_5,
				ARRAY_SIZE(initcode2_5)) == -1)
	printk("MIPI DSI failed to write CMD2_5\n");

	s5p_mipi_dsi_wr_data(dsim, MIPI_DSI_DCS_SHORT_WRITE_PARAM,
				initcode2_6[0], initcode2_6[0]);

	if (s5p_mipi_dsi_wr_data(dsim, MIPI_DSI_DCS_LONG_WRITE,
				(unsigned int)initcode2_7,
				ARRAY_SIZE(initcode2_7)) == -1)
	printk("MIPI DSI failed to write CMD2_7\n");
	if (s5p_mipi_dsi_wr_data(dsim, MIPI_DSI_DCS_LONG_WRITE,
				(unsigned int)initcode2_8,
				ARRAY_SIZE(initcode2_8)) == -1)
	printk("MIPI DSI failed to write CMD2_8\n");
	if (s5p_mipi_dsi_wr_data(dsim, MIPI_DSI_DCS_LONG_WRITE,
				(unsigned int)initcode2_9,
				ARRAY_SIZE(initcode2_9)) == -1)
	printk("MIPI DSI failed to write CMD2_9\n");

	s5p_mipi_dsi_wr_data(dsim, MIPI_DSI_DCS_SHORT_WRITE,
				0x35, 0);

	if (s5p_mipi_dsi_wr_data(dsim, MIPI_DSI_DCS_LONG_WRITE,
				(unsigned int)initcode2_11,
				ARRAY_SIZE(initcode2_11)) == -1)
	printk("MIPI DSI failed to write CMD2_11\n");

	/* gamma setting */
	if (s5p_mipi_dsi_wr_data(dsim, MIPI_DSI_DCS_LONG_WRITE,
				(unsigned int)initcode_d1,
				ARRAY_SIZE(initcode_d1)) == -1)
	printk("MIPI DSI failed to write CMDd_1\n");
	if (s5p_mipi_dsi_wr_data(dsim, MIPI_DSI_DCS_LONG_WRITE,
				(unsigned int)initcode_d2,
				ARRAY_SIZE(initcode_d2)) == -1)
	printk("MIPI DSI failed to write CMDd_2\n");
	if (s5p_mipi_dsi_wr_data(dsim, MIPI_DSI_DCS_LONG_WRITE,
				(unsigned int)initcode_d3,
				ARRAY_SIZE(initcode_d3)) == -1)
	printk("MIPI DSI failed to write CMDd_3\n");
	if (s5p_mipi_dsi_wr_data(dsim, MIPI_DSI_DCS_LONG_WRITE,
				(unsigned int)initcode_d4,
				ARRAY_SIZE(initcode_d4)) == -1)
	printk("MIPI DSI failed to write CMDd_4\n");
	if (s5p_mipi_dsi_wr_data(dsim, MIPI_DSI_DCS_LONG_WRITE,
				(unsigned int)initcode_d5,
				ARRAY_SIZE(initcode_d5)) == -1)
	printk("MIPI DSI failed to write CMDd_5\n");
	if (s5p_mipi_dsi_wr_data(dsim, MIPI_DSI_DCS_LONG_WRITE,
				(unsigned int)initcode_d6,
				ARRAY_SIZE(initcode_d6)) == -1)
	printk("MIPI DSI failed to write CMDd_6\n");
	/* display on */
	s5p_mipi_dsi_wr_data(dsim, MIPI_DSI_DCS_SHORT_WRITE,
				0x29, 0);
	msleep(10);
	...
	if (s5p_mipi_dsi_wr_data(dsim, MIPI_DSI_DCS_LONG_WRITE,
				...
				ARRAY_SIZE(initcode2_1)) == -1);
	if (s5p_mipi_dsi_wr_data(dsim, MIPI_DSI_DCS_LONG_WRITE,
				(unsigned int)initcode2_2,
				ARRAY_SIZE(initcode2_2)) == -1);
	if (s5p_mipi_dsi_wr_data(dsim, MIPI_DSI_DCS_LONG_WRITE,
				(unsigned int)initcode2_3,
