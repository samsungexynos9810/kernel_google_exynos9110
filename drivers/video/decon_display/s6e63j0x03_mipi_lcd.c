...
struct decon_lcd s6e63j0x03_lcd_info = {
	.mode = COMMAND_MODE,
...
static void init_lcd(struct mipi_dsim_device *dsim)
{
	...
	ret = s5p_mipi_dsi_rd_data(dsim, MIPI_DSI_DCS_READ, 0xDB, 1, &read_data[1]);
	...
	if (s5p_mipi_dsi_wr_data(dsim, MIPI_DSI_DCS_LONG_WRITE,
			(u32)TEST_KEY_ON_1, ARRAY_SIZE(TEST_KEY_ON_1)) == -1)
	dev_err(dsim->dev, "fail to send panel_condition_set command.\n");

	if (s5p_mipi_dsi_wr_data(dsim, MIPI_DSI_DCS_LONG_WRITE,
			(u32)TEST_KEY_ON_2, ARRAY_SIZE(TEST_KEY_ON_2)) == -1)
	dev_err(dsim->dev, "fail to send panel_condition_set command.\n");

	if (s5p_mipi_dsi_wr_data(dsim, MIPI_DSI_DCS_LONG_WRITE,
			(u32)PORCH_ADJUSTMENT, ARRAY_SIZE(PORCH_ADJUSTMENT)) == -1)
	dev_err(dsim->dev, "fail to send panel_condition_set command.\n");

	if (s5p_mipi_dsi_wr_data(dsim, MIPI_DSI_DCS_LONG_WRITE,
			(u32)FRAME_FREQ_SET, ARRAY_SIZE(FRAME_FREQ_SET)) == -1)
	dev_err(dsim->dev, "fail to send panel_condition_set command.\n");
	
	if (s5p_mipi_dsi_wr_data(dsim, MIPI_DSI_DCS_LONG_WRITE,
			(u32)MEM_ADDR_SET_0, ARRAY_SIZE(MEM_ADDR_SET_0)) == -1)
	dev_err(dsim->dev, "fail to send panel_condition_set command.\n");
	
	if (s5p_mipi_dsi_wr_data(dsim, MIPI_DSI_DCS_LONG_WRITE,
			(u32)MEM_ADDR_SET_1, ARRAY_SIZE(MEM_ADDR_SET_1)) == -1)
	dev_err(dsim->dev, "fail to send panel_condition_set command.\n");

	if (s5p_mipi_dsi_wr_data(dsim, MIPI_DSI_DCS_LONG_WRITE,
			(u32)LPTS_TIMMING_SET_0, ARRAY_SIZE(LPTS_TIMMING_SET_0)) == -1)
	dev_err(dsim->dev, "fail to send panel_condition_set command.\n");
	
	if (s5p_mipi_dsi_wr_data(dsim, MIPI_DSI_DCS_LONG_WRITE,
			(u32)LPTS_TIMMING_SET_1, ARRAY_SIZE(LPTS_TIMMING_SET_1)) == -1)
	dev_err(dsim->dev, "fail to send panel_condition_set command.\n");

	if (s5p_mipi_dsi_wr_data(dsim, MIPI_DSI_DCS_LONG_WRITE,
			(u32)SLEEP_OUT, ARRAY_SIZE(SLEEP_OUT)) == -1)
	dev_err(dsim->dev, "fail to send panel_condition_set command.\n");

	usleep_range(120000, 120000);

	if (s5p_mipi_dsi_wr_data(dsim, MIPI_DSI_DCS_LONG_WRITE,
			(u32)DEFAULT_WHITE_BRIGHTNESS, ARRAY_SIZE(DEFAULT_WHITE_BRIGHTNESS)) == -1)
	dev_err(dsim->dev, "fail to send panel_condition_set command.\n");

	if (s5p_mipi_dsi_wr_data(dsim, MIPI_DSI_DCS_LONG_WRITE,
			(u32)DEFAULT_WHITE_CTRL, ARRAY_SIZE(DEFAULT_WHITE_CTRL)) == -1)
	dev_err(dsim->dev, "fail to send panel_condition_set command.\n");
	
	if (s5p_mipi_dsi_wr_data(dsim, MIPI_DSI_DCS_LONG_WRITE,
			(u32)ACL_OFF, ARRAY_SIZE(ACL_OFF)) == -1)
	dev_err(dsim->dev, "fail to send panel_condition_set command.\n");

	if (s5p_mipi_dsi_wr_data(dsim, MIPI_DSI_DCS_LONG_WRITE,
			(u32)ELVSS_COND, ARRAY_SIZE(ELVSS_COND)) == -1)
	dev_err(dsim->dev, "fail to send panel_condition_set command.\n");
	
	if (s5p_mipi_dsi_wr_data(dsim, MIPI_DSI_DCS_LONG_WRITE,
			(u32)SET_POS, ARRAY_SIZE(SET_POS)) == -1)
	dev_err(dsim->dev, "fail to send panel_condition_set command.\n");

	if (s5p_mipi_dsi_wr_data(dsim, MIPI_DSI_DCS_LONG_WRITE,
			(u32)TE_ON, ARRAY_SIZE(TE_ON)) == -1)
	dev_err(dsim->dev, "fail to send panel_condition_set command.\n");

	if (s5p_mipi_dsi_wr_data(dsim, MIPI_DSI_DCS_LONG_WRITE,
			(u32)TEST_KEY_OFF_2, ARRAY_SIZE(TEST_KEY_OFF_2)) == -1)
	dev_err(dsim->dev, "fail to send panel_condition_set command.\n");

	if (s5p_mipi_dsi_wr_data(dsim, MIPI_DSI_DCS_LONG_WRITE,
			(u32)DISPLAY_ON, ARRAY_SIZE(DISPLAY_ON)) == -1)
	dev_err(dsim->dev, "fail to send panel_condition_set command.\n");
