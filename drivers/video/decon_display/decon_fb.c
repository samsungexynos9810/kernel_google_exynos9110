...
static void patch_set_shadow_protect(struct s3c_fb_win *win, struct s3c_fb *sfb, int enable)
{
	...
	if (enable) {
		if ((readl(sfb->regs + WINCON_SHADOW(0))) & 0x1) {
			while(1) {
...
static void __s3c_fb_update_regs(struct s3c_fb *sfb, struct s3c_reg_data *regs)
{
	...
	if (sfb->windows[0]->ready_to_dma) {
		data |= DECON_UPDATE_SLAVE_SYNC;
		sfb->windows[0]->ready_to_dma = false;
	}
...
static int parse_decon_win_params(struct device *dev)
{
	DT_READ_U32(dev->of_node, "left_margin",  decon_fb_win0.win_mode.left_margin);
	DT_READ_U32(dev->of_node, "right_margin", decon_fb_win0.win_mode.right_margin);
	DT_READ_U32(dev->of_node, "upper_margin", decon_fb_win0.win_mode.upper_margin);
	DT_READ_U32(dev->of_node, "lower_margin", decon_fb_win0.win_mode.lower_margin);
	DT_READ_U32(dev->of_node, "hsync_len",    decon_fb_win0.win_mode.hsync_len);
	DT_READ_U32(dev->of_node, "vsync_len",    decon_fb_win0.win_mode.vsync_len);
	DT_READ_U32(dev->of_node, "xres",         decon_fb_win0.win_mode.xres);
	DT_READ_U32(dev->of_node, "yres",         decon_fb_win0.win_mode.yres);
	DT_READ_U32(dev->of_node, "virtual_x",    decon_fb_win0.virtual_x);
	DT_READ_U32(dev->of_node, "virtual_y",    decon_fb_win0.virtual_y);
	DT_READ_U32(dev->of_node, "width",        decon_fb_win0.width);
	DT_READ_U32(dev->of_node, "height",       decon_fb_win0.height);
	DT_READ_U32(dev->of_node, "max_bpp",      decon_fb_win0.max_bpp);
	DT_READ_U32(dev->of_node, "default_bpp",  decon_fb_win0.default_bpp);
	return 0;
}
...
static int parse_plat_data(struct device *dev)
{
	...
	parse_decon_win_params(dev);
	decon_platdata.win[0] = &decon_fb_win0;
	decon_platdata.win[1] = &decon_fb_win0;
	decon_platdata.win[2] = &decon_fb_win0;
	decon_platdata.win[3] = &decon_fb_win0;
	decon_platdata.win[4] = &decon_fb_win0;
	return 0;
}
