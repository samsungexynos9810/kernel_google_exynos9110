...
#ifdef CONFIG_OF
static const struct of_device_id exynos_dsim[] = {
	{ .compatible = "samsung,exynos-dsim" },
	{},
};
MODULE_DEVICE_TABLE(of, exynos_dsim);
#endif
	 ...
	 u32 addr, u32 count, u8 *buf)
{
	...
	s5p_mipi_dsi_init_fifo_pointer(dsim, DSIM_INIT_RX);
	s5p_mipi_dsi_clear_interrupt(dsim,INTSRC_RX_DATA_DONE);
	...
	} while (!s5p_mipi_dsi_rx_fifo_is_empty(dsim) && --rx_fifo_depth);
...
int decon_mipi_dsi_set_dphy_timing_value(struct mipi_dsim_device *dsim)
{
