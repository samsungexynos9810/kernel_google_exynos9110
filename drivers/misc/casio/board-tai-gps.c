...
static int tai_gps_probe(struct platform_device *pdev)
{
	int ret, irq;
	...
	ret = alloc_chrdev_region(&devno, 0, 2, "tai_gps");
	if (ret < 0) {
		pr_err("%s: alloc_chrdev_region failed %d\n", __func__, ret);
		return ret;
	}
	gps_class = class_create(THIS_MODULE, "tai_gps");
	if (IS_ERR(gps_class)) {
	...
	wake_lock_init(&info->wakelock, WAKE_LOCK_SUSPEND, "tai_gps");
...
static struct of_device_id tai_gps_of_match[] = {
	{ .compatible = "tai-gps", },
	{ },
};

static struct platform_driver tai_gps_platform_driver = {
	.probe = tai_gps_probe,
	.driver = {
		.name = "tai-gps",
		.owner = THIS_MODULE,
		.of_match_table = of_match_ptr(tai_gps_of_match),
	},
};

module_platform_driver(tai_gps_platform_driver);

MODULE_ALIAS("platform:tai-gps");
MODULE_DESCRIPTION("tai_gps");
MODULE_AUTHOR("casio");
MODULE_LICENSE("GPL");
