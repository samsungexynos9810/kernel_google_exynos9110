...
static struct mtk_sdio_eint_platform_data mtk_sdio_eint_pdata = { 
	.sdio_eint = GPIO_COMBO_WIFI_EINT_PIN,  //MUST set pin num in target system. 
//	.sdio_eint = -EINVAL,
}; 
...
//	struct mtk_sdio_eint_platform_data *p = pdev->dev.platform_data;
	struct mtk_sdio_eint_platform_data *p = &mtk_sdio_eint_pdata;
	int ret = -EINVAL;
...
void mtk_hif_sdio_eint_init(void)
{
	...
	/* [2014.10.13 ray] added because of no probing automatically. why?? */
	hif_sdio_plt_probe(NULL);		// CYM
}
