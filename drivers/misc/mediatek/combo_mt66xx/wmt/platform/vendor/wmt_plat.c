...
*******************************************************************************/
extern int MTK6220_wifi_set_carddetect(int);
...
//UINT32 gWmtDbgLvl = WMT_LOG_LOUD;		// CYM - debug
...
#define GPIO_COMBO_RST_PIN 			52	// GPE1_4 : 52  : 6620_SYS_EN
#define GPIO_COMBO_EEDI_PIN 		53	// GPE1_5 : 53  : 6620_EEDI
#define GPIO_COMBO_PMU_EN_PIN 		54	// GPE1_6 : 54  : 6620_PMU_EN
#define GPIO_COMBO_BGF_EINT_PIN 	148	// GPX3_3 : 148 : 6620_BGF_INT
#define GPIO_COMBO_WIFI_EINT_PIN	149	// GPX3_4 : 149 : 6620_SDIO_INT

#define GPIO_COMBO_UART_RX 	  0		// GPA0_0 : 0 : BT_GPS_UART_RXD
#define GPIO_COMBO_UART_TX    1		// GPA0_1 : 1 : BT_GPS_UART_TXD


int wmt_init_gpio(void)
{
	int err;
...
//	printk("%s : start! \n", __func__);
	...
	//------------------------------------------------------
	/* EEDI_PIN */
	if (gpio_is_valid(GPIO_COMBO_EEDI_PIN)) {
		err = gpio_request(GPIO_COMBO_EEDI_PIN, "mtk_eedi");
		if (err) {
			printk("%s : eedi gpio request failed", __func__);
		}
...
//        gpio_direction_output(GPIO_COMBO_EEDI_PIN, 1);
		err = gpio_direction_output(GPIO_COMBO_EEDI_PIN, 1);
		if (err) {
			printk("%s : set_direction for eedi gpio failed", __func__);
		}
		gpio_set_value_cansleep(GPIO_COMBO_EEDI_PIN, 1);
		gpio_free(GPIO_COMBO_EEDI_PIN);
	}
	else {
		printk(KERN_ERR "mtk_wmt eedi pin(%u) is invalid\n", GPIO_COMBO_EEDI_PIN);
		return -1;
	}
	/* PMU_EN_PIN */
	if (gpio_is_valid(GPIO_COMBO_PMU_EN_PIN)) {
		err = gpio_request(GPIO_COMBO_PMU_EN_PIN, "mtk_pmu");
		if (err) {
			printk("%s : pmu gpio request failed", __func__);
		}
...
//        gpio_direction_output(GPIO_COMBO_PMU_EN_PIN, 0);
		err = gpio_direction_output(GPIO_COMBO_PMU_EN_PIN, 0);
		if (err) {
			printk("%s : set_direction for pmu gpio failed", __func__);
		}
		gpio_set_value_cansleep(GPIO_COMBO_PMU_EN_PIN, 1);
		gpio_free(GPIO_COMBO_PMU_EN_PIN);
	}
	else {
		printk(KERN_ERR "mtk_wmt pmu pin(%u) is invalid\n", GPIO_COMBO_PMU_EN_PIN);
		return -1;
	}
    /* RST_PIN */
	if (gpio_is_valid(GPIO_COMBO_RST_PIN)) {
		err = gpio_request(GPIO_COMBO_RST_PIN, "mtk_rst");
		if (err) {
...
//        gpio_direction_output(GPIO_COMBO_RST_PIN, 0);
		err = gpio_direction_output(GPIO_COMBO_RST_PIN, 0);
		if (err) {
			printk("%s : set_direction for reset gpio failed", __func__);
		}
		gpio_set_value_cansleep(GPIO_COMBO_RST_PIN, 1);
		gpio_free(GPIO_COMBO_RST_PIN);
	}
	else {
		printk(KERN_ERR "mtk_wmt rst pin(%u) is invalid\n", GPIO_COMBO_RST_PIN);
		return -1;
	}
    /* BGF_EINT_PIN */
	if (gpio_is_valid(GPIO_COMBO_BGF_EINT_PIN)) {
		err = gpio_request(GPIO_COMBO_BGF_EINT_PIN, "mtk_bgf_int");
		if (err) {
			printk("%s : bgf_int gpio request failed", __func__);
		}

        s3c_gpio_cfgpin(GPIO_COMBO_BGF_EINT_PIN, S3C_GPIO_SFN(0xF));
//		gpio_set_value_cansleep(GPIO_COMBO_BGF_EINT_PIN, 1);
		gpio_free(GPIO_COMBO_BGF_EINT_PIN);
	}
	else {
		printk(KERN_ERR "mtk_wmt bgf_int pin(%u) is invalid\n", GPIO_COMBO_BGF_EINT_PIN);
		return -1;
	}

	/* WIFI_EINT PIN */
	if (gpio_request(GPIO_COMBO_WIFI_EINT_PIN, "mtk_wifi_int")) {
		printk(KERN_ERR "mtk_wmt wifi_int pin(%u) is invalid\n", GPIO_COMBO_WIFI_EINT_PIN);
		return -1;
	}
...
#if 0
	/* UART RX */
	if (gpio_request(GPIO_COMBO_UART_RX, "uart_rx")) {
		printk(KERN_ERR "mtk_wmt bt pin0(%u) is invalid\n", GPIO_COMBO_UART_RX);
		return -1;
	}
    s3c_gpio_setpull(GPIO_COMBO_UART_RX, S3C_GPIO_PULL_UP);
    s5p_gpio_set_drvstr(GPIO_COMBO_UART_RX, S5P_GPIO_DRVSTR_LV4);
	gpio_free(GPIO_COMBO_UART_RX);

	/* UART TX */
	if (gpio_request(GPIO_COMBO_UART_TX, "uart_tx")) {
		printk(KERN_ERR "mtk_wmt bt pin1(%u) is invalid\n", GPIO_COMBO_UART_TX);
		return -1;
	}
    s3c_gpio_setpull(GPIO_COMBO_UART_TX, S3C_GPIO_PULL_UP);
    s5p_gpio_set_drvstr(GPIO_COMBO_UART_TX, S5P_GPIO_DRVSTR_LV4);
	gpio_free(GPIO_COMBO_UART_TX);
#endif
...
int wmmtk_combo_parse_dt(struct platform_device *pdev)
{
...
//		return 0;
	...
	//------------------------------------------------------
	/* EEDI_PIN */
	eedi_gpio = of_get_named_gpio(np, "mtk_wmt,eedi", 0);
	if (eedi_gpio < 0) {
		printk("Looking up %s property in node %s failed %d\n",
				"mtk_wmt,eedi", np->full_name,
				eedi_gpio);
		eedi_gpio = -1;
		return -1;
	}
	/* PMU_EN_PIN */
	wmt_pdata.pmu = of_get_named_gpio(np, "mtk_wmt,pmu", 0);
	if (wmt_pdata.pmu < 0) {
		printk("Looking up %s property in node %s failed %d\n",
				"mtk_wmt,pmu", np->full_name,
				wmt_pdata.pmu);
//		wmt_pdata.pmu = -1;
		return -1;
	}
    /* RST_PIN */
	wmt_pdata.rst = of_get_named_gpio(np, "mtk_wmt,rst", 0);
	if (wmt_pdata.rst < 0) {
		printk("Looking up %s property in node %s failed %d\n",
				"mtk_wmt,rst", np->full_name,
				wmt_pdata.rst);
		return -1;
	}
    /* BGF_EINT_PIN */
	wmt_pdata.bgf_int = of_get_named_gpio(np, "mtk_wmt,bgf_int", 0);
	if (wmt_pdata.bgf_int < 0) {
		printk("Looking up %s property in node %s failed %d\n",
				"mtk_wmt,bgf_int", np->full_name,
				wmt_pdata.bgf_int);
		return -1;
	}
	return 0;
...
void wmt_plat_gpio_init(struct platform_device *pdev)
{
	...
	// GPE1_4 : 52  : 6620_SYS_EN
	// GPE1_5 : 53  : 6620_EEDI
	// GPE1_6 : 54  : 6620_PMU_EN
	// GPX3_3 : 148 : 6620_BGF_INT

	// Init data
	wmt_pdata.pmu 		= GPIO_COMBO_PMU_EN_PIN;
	wmt_pdata.rst 		= GPIO_COMBO_RST_PIN;
	wmt_pdata.bgf_int 	= GPIO_COMBO_BGF_EINT_PIN;
	wmt_pdata.urt_cts 	= -EINVAL;
	wmt_pdata.rtc 		= -EINVAL;
	wmt_pdata.gps_sync	= -EINVAL;
	wmt_pdata.gps_lna 	= -EINVAL;

//	wmmtk_combo_parse_dt(pdev);
