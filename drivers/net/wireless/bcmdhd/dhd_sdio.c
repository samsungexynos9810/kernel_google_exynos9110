 ...
 *
 * $Id: dhd_sdio.c 666216 2017-08-04 10:02:19Z $
 */
...
#if (defined(SUPPORT_P2P_GO_PS) || !defined(OEM_ANDROID))
...
static int read_more_btbytes(struct dhd_bus *bus, void * file, int * addr_mode,
...
dhd_bus_watchdog(dhd_pub_t *dhdp)
{
	...
	if ((bus->idletime != 0) && (bus->clkstate == CLK_AVAIL) &&
		...
		if (++bus->idlecount >= bus->idletime) {
			...
			if (bus->activity) {
				bus->activity = FALSE;
#if !defined(OEM_ANDROID) && (!defined(NDISVER) || (NDISVER < 0x0630))
			} else {
#endif /* !defined(OEM_ANDROID) && !defined(NDISVER) */
				if (SLPAUTO_ENAB(bus)) {
					if (!bus->readframes)
						dhdsdio_bussleep(bus, TRUE);
					else
                     ...
                     uint16 devid)
{
	...
	/* Update F2 Blocksize to 256 for 43012 */
	if ((uint16)bus->sih->chip == BCM43012_CHIP_ID) {
		fn = 2;
		fn = fn << SDIO_FUNC_BLOCK_SIZE_SHIFT | F2_BLOCK_SIZE_256;
		if (bcmsdh_iovar_op(sdh, "sd_blocksize",
			NULL, 0, &fn, sizeof(fn), TRUE) != BCME_OK) {
...
#if !defined(BCMLXSDMMC) && !(defined(NDISVER) && (NDISVER >= 0x0650))
		if (bus->dhd) {
			dhdsdio_clkctl(bus, CLK_AVAIL, FALSE);
		}
...
static int
read_more_btbytes(struct dhd_bus *bus, void * file, int * addr_mode, uint16 * hi_addr,
...
_dhdsdio_download_btfw(struct dhd_bus *bus)
{
	...
	while (read_more_btbytes (bus, image, &addr_mode, &hiAddress, &dest_addr,
		data_ptr, &num_bytes)) {
