 ...
 *
 * $Id: wlioctl.h 663416 2017-01-04 03:15:53Z $
 */
...
typedef struct wlc_rev_info {
	...
	uint		coreminorrev;	/* phy minor rev */
} wlc_rev_info_t;
...
typedef BWL_PRE_PACKED_STRUCT struct ocl_status_info {
	uint8  version;
	uint8  len;
...
} BWL_POST_PACKED_STRUCT ocl_status_info_t;

/* MWS OCL map */
#define WL_MWS_OCL_OVERRIDE_VERSION 1
typedef BWL_PRE_PACKED_STRUCT struct wl_mws_ocl_override {
	uint16  version;    /* Structure version */
	...
	uint16	bitmap_5g_high; /* bitmap for 5G high channels by 2
				* 153, 157-161, 165
				*/
} BWL_POST_PACKED_STRUCT wl_mws_ocl_override_t;
...
typedef BWL_PRE_PACKED_STRUCT struct wl_mimo_meas_metrics_v1 {
	uint16 type;
	uint16 len;
...
} BWL_POST_PACKED_STRUCT wl_mimo_meas_metrics_v1_t;
...
#define WL_MIMO_SISO_METRICS_SIZE	(sizeof(wl_mimo_meas_metrics_v1_t))
#define WL_MIMO_SISO_OCL_METRICS_SIZE	(sizeof(wl_mimo_meas_metrics_t))
...
/*
 * Sub-command optional parameters packed into tlv_t type container
 */
...
typedef struct wl_nan_ev_receive {
	wl_nan_instance_id_t local_id;
	wl_nan_instance_id_t remote_id;
...
typedef struct wl_nan_ndp_dataresp
{
	uint8 ndp_id;
	uint8 status;
	uint8 pad[2];
} wl_nan_ndp_dataresp_t;
...
typedef struct wl_nan_ev_data_end {
	uint8 ndp_id;
	uint8 status;
	/* struct ether_addr init_data_addr; */
	/* struct ether_addr peer_data_addr; */
	uint8 pad[2];
...
enum {
	...
	WL_AVAIL_UAW		= 0x0004	/* unaligned schedule */
};
...
enum {
	...
	WL_PROXD_EVENT_START_WAIT	= 17,	/* waiting to start */
...
enum wl_mbo_cmd_ids {
	...
	/* Add before this !! */
	WL_MBO_CMD_LAST = WL_MBO_CMD_CLEAR_COUNTERS
};
