 ...
 *
 * $Id: wl_cfg80211.c 666367 2017-08-16 07:10:36Z $
 */
/* */
#include <typedefs.h>
#include <linuxver.h>
...
static s8 wl_dbg_estr[][WL_DBG_ESTR_MAX] = {
	...
	"WLC_E_CCX_ASSOC_START", "WLC_E_CCX_ASSOC_ABORT",
#else
	...
	enum nl80211_iftype iface_type, s32 del, u8 *addr)
{
	...
	if (ret < 0)
		WL_DBG(("Interface %s failed!! ret %d\n",
	...
	u8 *mac_addr, const char *name)
{
	...
	if (iface_type == NL80211_IFTYPE_STATION) {
		...
		WL_ERR(("Vif not supported for iface type:%d \n",
			iface_type));
		goto fail;
	}
	...
	struct cfg80211_ibss_params *params)
{
	...
	if (wl_get_drv_status(cfg, CONNECTED, dev)) {
		...
		chanspec_t *chspec = (chanspec_t *)wl_read_prof(cfg, dev, WL_PROF_CHANSPEC);
		u32 channel = (u32)CHSPEC_CHANNEL(*chspec);
	...
	wl_update_prof(cfg, dev, NULL, &chanspec, WL_PROF_CHANSPEC);
#ifdef WLAIBSS
...
#define DEFAULT_5G_SOFTAP_CHANNEL	149
	if (wl_get_mode_by_netdev(cfg, dev) == WL_MODE_AP &&
		...
		chanspec_t *chanspec = (chanspec_t *)wl_read_prof(cfg, bcmcfg_to_prmry_ndev(cfg),
			WL_PROF_CHANSPEC);
		u32 sta_chan = wf_chspec_ctlchan(*chanspec);
	...
	struct net_device *dev)
{
	...
	if (unlikely(err)) {
		if (err == BCME_UNSUPPORTED) {
			...
			WL_DBG(("scb_probe is unsupported\n"));
		} else {
...
#endif /* (LINUX_VERSION_CODE > KERNEL_VERSION(3, 14, 0)) || defined(WL_VENDOR_EXT_SUPPORT) */
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 11, 0))
		/* Reset wowlan before Unregister to avoid  Kernel Panic */
...
static s32 wl_update_bss_info(struct bcm_cfg80211 *cfg, struct net_device *ndev, bool roam)
{
	...
	*(u32 *) cfg->extra_buf = htod32(WL_EXTRA_BUF_MAX);
	...
	bi = (struct wl_bss_info *)(cfg->extra_buf + 4);
	chanspec = wl_chspec_driver_to_host(bi->chanspec);
	wl_update_prof(cfg, ndev, NULL, &chanspec, WL_PROF_CHANSPEC);
	...
	const wl_event_msg_t *e, void *data)
{
	 ...
	 */
	curbssid = wl_read_prof(cfg, ndev, WL_PROF_BSSID);
	chanspec = (chanspec_t *)wl_read_prof(cfg, ndev, WL_PROF_CHANSPEC);
	channel = CHSPEC_CHANNEL(*chanspec);
	...
	enum wl_status state, bool set)
{
	...
	if (set) {
		...
		chanspec = 0;
		/* clear chanspec information when the net device is disconnected */
		wl_update_prof(cfg, _net_info->ndev, NULL, &chanspec, WL_PROF_CHANSPEC);
		wl_cfg80211_determine_vsdb_mode(cfg);
...
static void *wl_read_prof(struct bcm_cfg80211 *cfg, struct net_device *ndev, s32 item)
{
	...
	case WL_PROF_SSID:
		rptr = &profile->ssid;
		break;
	case WL_PROF_CHANSPEC:
		rptr = &profile->chanspec;
		break;
	}
	spin_unlock_irqrestore(&cfg->cfgdrv_lock, flags);
	if (!rptr)
	...
	const wl_event_msg_t *e, const void *data, s32 item)
{
	...
	case WL_PROF_DTIMPERIOD:
		...
		break;
	case WL_PROF_CHANSPEC:
		profile->chanspec = *(const chanspec_t *)data;
		break;
	default:
		err = -EOPNOTSUPP;
		break;
...
_Pragma("GCC diagnostic ignored \"-Wcast-qual\"")
#endif
	while ((ie = bcm_parse_tlvs((void *)parse, (int)len, DOT11_MNG_INTERWORKING_ID))) {
		return ie;
	}
#if defined(STRICT_GCC_WARNINGS) && defined(__GNUC__) && (__GNUC__ > 4 || (__GNUC__ == \
	4 && __GNUC_MINOR__ >= 6))
...
wl_cfg80211_tdls_config(struct bcm_cfg80211 *cfg, enum wl_tdls_config state, bool auto_mode)
{
	...
	if ((state == TDLS_STATE_TEARDOWN)) {
		...
		return dhd_tdls_enable(ndev, false, auto_mode, NULL);
...
_wl_update_ap_rps_params(struct net_device *dev)
{
	...
	if (wldev_iovar_setint(dev, "radio_pwrsave_pps", cfg->ap_rps_info.pps)) {
		WL_ERR(("Failed to set radio pps"));
		return BCME_ERROR;
	}

	if (wldev_iovar_setint(dev, "radio_pwrsave_stas_assoc_check",
		cfg->ap_rps_info.sta_assoc_check)) {
		WL_ERR(("Failed to set radio stas assoc check"));
		return BCME_ERROR;
	}

	if (wldev_iovar_setint(dev, "radio_pwrsave_quiet_time",
		cfg->ap_rps_info.quiet_time)) {
		WL_ERR(("Failed to set radio quiettime"));
		return BCME_ERROR;
	}

	if (wldev_iovar_setint(dev, "radio_pwrsave_noalevel", cfg->ap_rps_info.level)) {
		WL_ERR(("Failed to set radio level"));
		return BCME_ERROR;
	}
...
wl_get_ap_rps(struct net_device *dev, char* command, char *ifname, int total_len)
{
	...
	error = wldev_iovar_getint(ndev, "radio_pwrsave", &enable);
	if (error < 0) {
		WL_ERR(("get ap radio pwrsave failed = %d\n", error));
		return error;
	}

	bytes_written += snprintf(command + bytes_written, total_len, "%d", enable);
...
wl_set_ap_rps(struct net_device *dev, bool enable, char *ifname)
{
	...
	if (cfg->ap_rps_info.enable != enable) {
		...
		if (wldev_iovar_setint(ndev, "radio_pwrsave_enable", enable)) {
...
wl_update_ap_rps_params(struct net_device *dev, ap_rps_info_t* rps, char *ifname)
{
	...
	if (cfg->ap_rps_info.pps != rps->pps) {
		cfg->ap_rps_info.pps = rps->pps;
		if (cfg->ap_rps_info.enable) {
			if (wldev_iovar_setint(ndev, "radio_pwrsave_pps", rps->pps)) {
				WL_ERR(("Failed to set radio pps"));
				return BCME_ERROR;
			}
		}
	...
	if (cfg->ap_rps_info.level != rps->level) {
		cfg->ap_rps_info.level = rps->level;
		if (cfg->ap_rps_info.enable) {
			if (wldev_iovar_setint(ndev, "radio_pwrsave_noalevel",
				rps->level)) {
				WL_ERR(("Failed to set radio level"));
				return BCME_ERROR;
			}
	...
	if (cfg->ap_rps_info.quiet_time != rps->quiet_time) {
		cfg->ap_rps_info.quiet_time = rps->quiet_time;
		if (cfg->ap_rps_info.enable) {
			if (wldev_iovar_setint(ndev, "radio_pwrsave_quiet_time",
				rps->quiet_time)) {
				WL_ERR(("Failed to set radio quiet time"));
				return BCME_ERROR;
			}
		}
	}

	if (cfg->ap_rps_info.sta_assoc_check != rps->sta_assoc_check) {
		cfg->ap_rps_info.sta_assoc_check = rps->sta_assoc_check;
		if (cfg->ap_rps_info.enable) {
			if (wldev_iovar_setint(ndev, "radio_pwrsave_stas_assoc_check",
				rps->sta_assoc_check)) {
				WL_ERR(("Failed to set radio sta assoc check"));
				return BCME_ERROR;
			}
		}
	}
