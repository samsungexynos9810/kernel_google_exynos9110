 ...
 *
 * $Id: bcmsdh_sdmmc_linux.c 664392 2017-03-24 10:42:39Z $
 */
...
int bcmsdh_sdmmc_set_power(int on)
{
	...
	if (gfunc) {
		...
		if (on) {
			sd_err(("bcmsdh_sdmmc_set_power : Calling mmc_power_restore_host\n"));
			mmc_power_restore_host(sdio_func->card->host);
		}
		else {
			sd_err(("bcmsdh_sdmmc_set_power : Calling mmc_power_save_host\n"));
			mmc_power_save_host(sdio_func->card->host);
		}
	}
	return 0;
}
#endif /* POWER_OFF_MMC_HOST */
