 ...
 *
 * $Id: dhd_linux.c 666367 2017-08-16 07:10:36Z $
 */
...
#if !defined(BCMDHDUSB)
			/* The below change to defer the processing of trace in workQueue
			 * is temporarily disabled for usb chip. As the previous implementation
			 * was tested in earlier usb chips. Can be reverted after a similar
			 * workQueue implementation was done for usb chips
			 */
#ifdef SHOW_LOGTRACE
			/* Event msg printing is called from dhd_rx_frame which is in Tasklet
			 * context in case of PCIe FD, in case of other bus this will be from
...
#elif defined(RSDB_MODE_FROM_FILE)
	(void)dhd_rsdb_mode_from_file(dhd);
#endif /* RSDB_MODE_DEFAULT_OFF */
	...
	u8* program, uint32 program_len)
{
	...
	if (program_len > APF_PROGRAM_MAX_SIZE) {
...
#if defined(OEM_ANDROID) && defined(BOARD_PANDA)
