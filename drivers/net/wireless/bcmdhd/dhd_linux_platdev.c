 ...
 *
 * $Id: dhd_linux_platdev.c 664823 2017-04-26 09:31:43Z $
 */
#include <typedefs.h>
...
static int wifi_get_external_mac_addr(unsigned char *buf)
{
	...
	char fname[] = "/device_config/wifi/mac.txt";
...
#ifdef POWER_OFF_MMC_HOST
	if (!adapter->wifi_plat_data) {
		DHD_ERROR(("%s: unable to get platform data !\n", __FUNCTION__));
		return -ENODATA;
	}
	((struct wifi_platform_data *)(adapter->wifi_plat_data))->set_power =
		bcmsdh_sdmmc_set_power;
#endif

#ifdef CUSTOMER_INTEL
	((struct wifi_platform_data *)(adapter->wifi_plat_data))->get_mac_addr =
		wifi_get_external_mac_addr;
#endif
