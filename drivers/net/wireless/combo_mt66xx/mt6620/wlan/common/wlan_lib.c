...
// nermy
        ...
        if(prAdapter->fgIsEmbbededMacAddrValid == TRUE) {
            ...
            DBGLOG(INIT, STATE, ("WIFI_CONFIG:Using eFuse MAC address\n"));
#endif
            return WLAN_STATUS_SUCCESS;
        }
        else  
...
#if CFG_SHOW_MACADDR_SOURCE
        DBGLOG(INIT, STATE, ("WIFI_CONFIG:Using NVRAM MAC address\n"));
#endif
    }
    ...
    )
{
    ...
    if(prAdapter->rVerInfo.u2Part1CfgOwnVersion == 0x0001) {
		...
		DBGLOG(INIT, STATE, ("WIFI_CONFIG:NVRAM TX power is used \n"));
    }
    else {
    ...
    if(prRegInfo->ucEnable5GBand) {
        ...
        if(prAdapter->fgIsHw5GBandDisabled
			...
			DBGLOG(INIT, STATE, ("WIFI_CONFIG:5G is not enabled \n"));
        }
        else {
            prAdapter->fgEnable5GBand = TRUE;
			DBGLOG(INIT, STATE, ("WIFI_CONFIG:5G is enabled \n"));
        }
    }
    else {
        prAdapter->fgEnable5GBand = FALSE;
