...
static void s2mm001_print_reg_log(void)
{
	...
	for (i = 0; i < MAX_LOG; i++) {
		...
		if (s2mm001_log_cnt >= MAX_LOG)
			s2mm001_log_cnt = 0;
		snprintf(mesg+strlen(mesg), sizeof(mesg) - strlen(mesg),
					  "%x(%x)%x ", reg, value, rw);
	}
	pr_info("%s:%s\n", __func__, mesg);
}
...
static int s2mm001_set_gpio_uart_sel(int uart_sel)
{
	...
	case MUIC_PATH_UART_AP:
		...
		if (gpio_is_valid(uart_sel_gpio)) {
			if (muic_pdata.gpio_revers)
				gpio_direction_output(uart_sel_gpio,
						     GPIO_LEVEL_LOW);
			else
				gpio_direction_output(uart_sel_gpio,
						    GPIO_LEVEL_HIGH);
					   ...
					   char *buf)
{
	...
	case MUIC_PATH_UART_AP:
		pr_info("%s:%s AP\n", MUIC_DEV_NAME, __func__);
		return snprintf(buf, 20, "AP\n");
	case MUIC_PATH_UART_CP:
		pr_info("%s:%s CP\n", MUIC_DEV_NAME, __func__);
		return snprintf(buf, 20, "CP\n");
	default:
		break;
	}
					   ...
					   char *buf)
{
	...
	case MUIC_PATH_USB_AP:
		return snprintf(buf, 20, "PDA\n");
	case MUIC_PATH_USB_CP:
		return snprintf(buf, 20, "MODEM\n");
	default:
		break;
	}
					   ...
					   char *buf)
{
	...
	return snprintf(buf, 256, "%s:%s attached_dev = %d\n",
					    ...
					    char *buf)
{
	...
	case ATTACHED_DEV_JIG_USB_ON_MUIC:
		return snprintf(buf, 32, "USB_STATE_CONFIGURED\n");
	default:
		break;
	}
					   ...
					   char *buf)
{
	...
	return snprintf(buf, 64, "st1:0x%x st2:0x%x\n", st1, st2);
}
					 ...
					 char *buf)
{
	...
	case ATTACHED_DEV_NONE_MUIC:
		return snprintf(buf, 20, "No VPS\n");
	case ATTACHED_DEV_USB_MUIC:
		return snprintf(buf, 20, "USB\n");
	case ATTACHED_DEV_CDP_MUIC:
		return snprintf(buf, 20, "CDP\n");
	case ATTACHED_DEV_OTG_MUIC:
		return snprintf(buf, 20,  "OTG\n");
	case ATTACHED_DEV_TA_MUIC:
		return snprintf(buf, 20, "TA\n");
	case ATTACHED_DEV_JIG_UART_OFF_MUIC:
		return snprintf(buf, 32, "JIG UART OFF\n");
	case ATTACHED_DEV_JIG_UART_OFF_VB_MUIC:
		return snprintf(buf, 32, "JIG UART OFF/VB\n");
	case ATTACHED_DEV_JIG_UART_ON_MUIC:
		return snprintf(buf, 32, "JIG UART ON\n");
	case ATTACHED_DEV_JIG_USB_OFF_MUIC:
		return snprintf(buf, 32, "JIG USB OFF\n");
	case ATTACHED_DEV_JIG_USB_ON_MUIC:
		return snprintf(buf, 32, "JIG USB ON\n");
	case ATTACHED_DEV_DESKDOCK_MUIC:
		return snprintf(buf, 32, "DESKDOCK\n");
	case ATTACHED_DEV_AUDIODOCK_MUIC:
		return snprintf(buf, 32, "AUDIODOCK\n");
	default:
		break;
	}

	return snprintf(buf, 32, "UNKNOWN\n");
}
...
static int com_to_open(struct s2mm001_muic_data *muic_data)
{
	...
	ret = s2mm001_i2c_read_byte(muic_data->i2c, 0x24);
	ret &= ~(0x01 << 4);
	s2mm001_i2c_write_byte(muic_data->i2c, 0x24, ret);

	ret = s2mm001_i2c_read_byte(muic_data->i2c, S2MM001_MUIC_REG_INTMASK2);
	ret &= ~0x01;
	s2mm001_i2c_write_byte(muic_data->i2c, S2MM001_MUIC_REG_INTMASK2, ret);
...
#endif
	reg_val = s2mm001_i2c_read_byte(muic_data->i2c, 0x24);
	reg_val |= 0x01 << 4;
	s2mm001_i2c_write_byte(muic_data->i2c, 0x24, reg_val);

	reg_val = s2mm001_i2c_read_byte(muic_data->i2c,
				S2MM001_MUIC_REG_INTMASK2);
	reg_val |= 0x01;
	s2mm001_i2c_write_byte(muic_data->i2c,
			S2MM001_MUIC_REG_INTMASK2, reg_val);
			...
			struct s2mm001_muic_data *muic_data)
{
	...
	if (np_muic == NULL) {
		...
		if (of_get_property(np_muic, "gpio-revers", NULL))
			muic_data->pdata->gpio_revers = true;
		else
			muic_data->pdata->gpio_revers = false;
	}
...
static const struct i2c_device_id s2mm001_muic_i2c_id[] = {
	{ MUIC_DEV_NAME, 0 },
	{}
};
MODULE_DEVICE_TABLE(i2c, s2mm001_muic_i2c_id);
...
#if defined(CONFIG_OF)
static struct of_device_id s2mm001_muic_match_table[] = {
	{ .compatible = "samsung,muic-s2mm001" },
	{ }
};
MODULE_DEVICE_TABLE(of, s2mm001_muic_match_table);
#endif /* CONFIG_OF */
