...
int muic_set_safeout(int safeout_path)
{
	...
	pr_debug("%s:MUIC safeout path=%d\n", __func__, safeout_path);
	...
	if (safeout_path == MUIC_PATH_USB_CP) {
		...
		pr_debug("%s: not control safeout(%d)\n", __func__, safeout_path);
		return -EINVAL;
	}

	return 0;
err:
	pr_debug("%s: cannot regulator_enable (%d)\n", __func__, ret);
	regulator_put(regulator);
	return ret;
}
...
static int muic_init_gpio_cb(int switch_sel)
{
	...
	pr_debug("%s: usb_path(%s), uart_path(%s), rustproof(%c)\n", __func__, usb_mode,
