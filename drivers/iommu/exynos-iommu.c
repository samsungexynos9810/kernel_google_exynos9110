				   ...
				   struct device *dev)
{
	struct exynos_iommu_owner *owner = dev->archdata.iommu;
	struct exynos_iommu_domain *priv = domain->priv;
	phys_addr_t phys = __pa(priv->pgtable);
	unsigned long flags;
	int ret;

	spin_lock_irqsave(&priv->lock, flags);

	ret = __exynos_sysmmu_enable(dev, phys, domain);
				    ...
				    struct device *dev)
{
	...
	if (owner == dev->archdata.iommu) {
		...
		TRACE_LOG_DEV(dev, "%s: Detached IOMMU\n", __func__);
	} else {
...
static int sysmmu_pm_suspend(struct device *dev)
{
	struct sysmmu_list_data *list;
