...
void melfas_vdd_pullup(bool on)
{
	...
	regulator_vdd = regulator_get(NULL, "vdd_tsp_1.8");
...
int melfas_power(bool on)
{
	...
	regulator_vdd = regulator_get(NULL, "vdd_tsp_3.3");
	if (IS_ERR(regulator_vdd))
...
int is_melfas_vdd_on(void)
{
	...
	if (!regulator) {
		regulator = regulator_get(NULL, "vdd_tsp_3.3");
		if (IS_ERR(regulator)) {
...
#ifdef CONFIG_OF
static struct melfas_tsi_platform_data *mms134_parse_dt(struct device *dev)
{
	struct melfas_tsi_platform_data *pdata;
	...
	pdata->power = melfas_power;
	pdata->is_vdd_on = is_melfas_vdd_on;
	pdata->keyled = key_led_control;
	pdata->register_cb = melfas_register_charger_callback;

	return pdata;
}
#else
static inline struct melfas_tsi_platform_data *mms134_parse_dt(struct device *dev)
{
	return NULL;
}
#endif
				  ...
				  const struct i2c_device_id *id)
{
	...
	if (!pdata)
		pdata = mms134_parse_dt(&client->dev);
...
#ifdef CONFIG_OF
static struct of_device_id mms134_dt_match[] = {
...
static struct i2c_driver mms_ts_driver = {
	...
	.driver = {
		   .name = MELFAS_TS_NAME,
		   .of_match_table = of_match_ptr(mms134_dt_match),
#if defined(CONFIG_PM) && !defined(CONFIG_HAS_EARLYSUSPEND)
		   .pm = &mms_ts_pm_ops,
#endif
	},
