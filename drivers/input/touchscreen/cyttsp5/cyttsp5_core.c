...
static int cyttsp5_core_wake_device_(struct cyttsp5_core_data *cd)
{
	if (!IS_DEEP_SLEEP_CONFIGURED(cd->easy_wakeup_gesture)) {
		...
		pinctrl_get_select(&cd->dev, "wakeup");
		udelay(10);
		pinctrl_get_select(&cd->dev, "default");
	}*/
		...
		u16 irq, size_t xfer_buf_size)
{
	...
	wake_lock_init(&cd->touch_wake_lock,
		WAKE_LOCK_SUSPEND, "touch_wake_lock");
...
error_startup:
	...
	cyttsp5_stop_wd_timer(cd);
	cyttsp5_free_si_ptrs(cd);
	wake_lock_destroy(&cd->touch_wake_lock);
