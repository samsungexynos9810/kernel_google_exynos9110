...
#define CONFIG_LCD_SHIRI
#define TOUCH_VER_140605

#define SCREEN_MAX_X			240
#define SCREEN_MAX_Y			240
#define TSP_MAX_X			240
#define TSP_MAX_Y			240
...
static int wakeup_from_touch = 0;
#endif
...
#if defined(TOUCH_VER_140605) /* (X, Y) = (max_x - x, max_y - y) */
/*		gprintk("x,y = %d, %d\n", x, y); */
...
/*		gprintk("x,y = %d, %d\n", x, y); */
		...
		/* invalid position */
		if ((x < 0)||(x > TSP_MAX_X)||(y < 0)||(y > TSP_MAX_Y))
			break;
		...
		if (status == FT_TOUCH_DOWN || status == FT_TOUCH_CONTACT) {
			...
			dev_printk(KERN_DEBUG, &data->client->dev,
				"Released touch point%d [ %d , %d ]\n",id ,x ,y);
...
static int ft5x06_power_on(struct ft5x06_ts_data *data, bool on)
{
	...
	printk( "[ TOUCH ] ft5x06_power_on : %d \n", on);
...
static int ft5x06_power_init(struct ft5x06_ts_data *data, bool on)
{
	...
	data->vdd = regulator_get(&data->client->dev, "vdd_tsp_3.3");
	if (IS_ERR(data->vdd)) {
		rc = PTR_ERR(data->vdd);
		dev_err(&data->client->dev,
...
#if ((!defined(CONFIG_FB) && !defined(CONFIG_HAS_EARLYSUSPEND)) || defined(CONFIG_TOUCHSCREEN_ENABLE_WAKEUP))
...
//		usleep(FT_55_AA_DLY_NS);
		usleep_range(FT_55_AA_DLY_NS, FT_55_AA_DLY_NS*2);
