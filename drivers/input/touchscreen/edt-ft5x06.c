...
//			continue;
		...
		dev_info(dev, "point%d type%d  X:%d , Y:%d \n", id, type, x, y);
...
//			input_report_abs(tsdata->input, ABS_MT_TOUCH_MAJOR, 0);
			dev_info(dev, "touch released \n");
		//	continue;
		}

		if(x < tsdata->num_x && y < tsdata->num_y)
		{
...
//			input_report_abs(tsdata->input, ABS_MT_TOUCH_MAJOR, 50);
		}
				...
				struct edt_ft5x06_ts_data *tsdata)
{
	...
	if (of_property_read_u32(np, "max_x", &tsdata->num_x)) {
		dev_err(dev, "failed to get max_x property\n");
		return -EINVAL;
	};
	if (of_property_read_u32(np, "max_y", &tsdata->num_y)) {
		dev_err(dev, "failed to get max_y property\n");
		return -EINVAL;
	};
	dev_info(dev,
		"reset_pin:%d  wake_pin:%d  num_x:%d  num_y:%d\n",
...
//	input_set_abs_params(input, ABS_MT_TOUCH_MAJOR, 0, MAX_PRESSURE, 0, 0);
