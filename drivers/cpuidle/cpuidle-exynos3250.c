...
#include "../../arch/arm/mach-exynos/include/mach/smc.h"
#include "../../arch/arm/mach-exynos/regs-pmu.h"
#include "../../arch/arm/mach-exynos/common.h"
#include "../../arch/arm/plat-samsung/include/plat/cpu.h"
...
#define EXYNOS3_CLKGATE_IP_MFC		0xC928
#define EXYNOS3_CLKGATE_IP_PERIL	0xC950
#define EXYNOS3_PWR_CTRL		0x15020
#define EXYNOS3_PWR_CTRL2		0x15024
...
static inline u32 cmu_raw_readl(u32 offset)
{
	return __raw_readl(reg_base + offset);
}
...
static struct check_reg_lpa exynos_clock_gating[] = {
	...
	{.check_reg = EXYNOS3_CLKGATE_IP_PERIL,	.check_bit = 0x00013FC0},
};
...
static int exynos_check_reg_status(struct check_reg_lpa *reg_list,
				    unsigned int list_cnt, int type)
{
	unsigned int i;
	unsigned int tmp;
	...
	for (i = 0; i < list_cnt; i++) {
		if (type == REG_TYPE_PMU)
			tmp = pmu_raw_readl(reg_list[i].check_reg);
		else if (type == REG_TYPE_CMU)
			tmp = cmu_raw_readl(reg_list[i].check_reg);
...
static int __maybe_unused exynos_check_enter_mode(void)
{
	...
	if (exynos_check_reg_status(exynos_power_domain,
			    ARRAY_SIZE(exynos_power_domain), REG_TYPE_PMU))
		return EXYNOS_CHECK_DIDLE;

	/* Check clock gating */
	if (exynos_check_reg_status(exynos_clock_gating,
			    ARRAY_SIZE(exynos_clock_gating), REG_TYPE_CMU))
		return EXYNOS_CHECK_DIDLE;
...
void exynos3250_enable_idle_clock_down(void)
{
	...
	tmp = cmu_raw_readl(EXYNOS3_PWR_CTRL);
	tmp &= ~((0x7 << 28) | (0x7 << 16) | (1 << 9) | (1 << 8));
	tmp |= (0x7 << 28) | (0x7 << 16) | 0x3ff;
	cmu_raw_writel(tmp, EXYNOS3_PWR_CTRL);

	tmp = cmu_raw_readl(EXYNOS3_PWR_CTRL2);
	tmp &= ~((0x3 << 24) | (0xffff << 8) | (0x77));
	tmp |= (100 << 16) | (10 << 8) | (1 << 4) | (0 << 0);
	tmp |= (1 << 25) | (1 << 24);
	cmu_raw_writel(tmp, EXYNOS3_PWR_CTRL2);
}
...
{
	unsigned int origin = pmu_raw_readl(EXYNOS_WAKEUP_MASK);

	origin = (origin & ~((0x1<<14)|(0x1<<5)|(0x1<<1))) | (0x1 << 30);
	pmu_raw_writel(origin, EXYNOS_WAKEUP_MASK);
	pmu_raw_writel(0x0, EXYNOS_WAKEUP_MASK2);
}
				...
				int index)
{
	...
	tmp = pmu_raw_readl(EXYNOS_CENTRAL_SEQ_CONFIGURATION);
	...
	pmu_raw_writel(0x0, EXYNOS_WAKEUP_STAT);
				...
				int index)
{
	...
	pmu_raw_writel(0xff6dffdb, EXYNOS_EINT_WAKEUP_MASK);

	exynos3250_disable_idle_clock_down();

	if (exynos_check_reg_status(audio_clock_gating,
			ARRAY_SIZE(audio_clock_gating), REG_TYPE_CMU))
...
static int __init exynos_init_cpuidle(void)
{
	...
	pmu_raw_writel((0x1<<14)|(0x3<<9)|(0x1<<5)|(0x3<<1), EXYNOS_WAKEUP_MASK);

	pr_info("CPUidle initialize complete\n");
