/* drivers/gpu/t6xx/kbase/src/platform/5430/gpu_control.c
 *
...
{
	if (!platform->clk_g3d) {
		GPU_LOG(DVFS_ERROR, "clk_g3d is not initialized\n");
		return -1;
	}

	platform->cur_clock = clk_get_rate(platform->clk_g3d)/MHZ;
	return 0;
}
...
static int gpu_clock_on(struct exynos_context *platform)
{
	...
	if (platform->clk_g3d)
		(void) clk_enable(platform->clk_g3d);
...
static int gpu_clock_off(struct exynos_context *platform)
{
	...
	if (platform->clk_g3d)
		(void)clk_disable(platform->clk_g3d);
...
static int gpu_set_clock(struct exynos_context *platform, int freq)
{
	...
	g3d_rate_prev = exynos_get_rate("fout_g3d_pll");
	...
	if (g3d_rate != g3d_rate_prev) {
		...
		if (ret < 0) {
			GPU_LOG(DVFS_ERROR, "failed to exynos_set_rate [fout_g3d_pll]\n");
			goto err;
		}
...
static int gpu_get_clock(struct exynos_context *platform)
{
	platform->clk_g3d = __clk_lookup("dout_aclk_g3d");
	clk_prepare(platform->clk_g3d);
	if (IS_ERR(platform->clk_g3d)) {
		platform->clk_g3d = NULL;
		GPU_LOG(DVFS_ERROR, "failed to __clk_lookup [g3d]\n");
	}

	return 0;
}

static int gpu_clock_init(struct exynos_context *platform)
{
	int ret;

	ret = gpu_get_clock(platform);
	if (ret < 0)
		return -1;
