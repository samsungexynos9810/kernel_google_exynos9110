...
static int kbase_platform_power_clock_init(kbase_device *kbdev)
{
	...
	/* Turn on G3D power */
	__raw_writel(0x7, EXYNOS5422_G3D_CONFIGURATION);

	/* Wait for G3D power stability for 1ms */
	timeout = 10;
	while ((__raw_readl(EXYNOS5422_G3D_STATUS) & 0x7) != 0x7) {
		if (timeout == 0) {
			/* need to call panic  */
...
{
	return ((__raw_readl(EXYNOS5422_G3D_STATUS) & 0x7) == 0x7) ? 1 : 0;
}
...
static int kbase_platform_power_off(void)
{
	...
	while (__raw_readl(EXYNOS5422_G3D_STATUS) & 0x7) {
		if (timeout == 0) {
			/* need to call panic */
