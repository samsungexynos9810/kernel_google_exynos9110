...
int mali_module_init(void)
{
	...
	printk("Inserting Mali v%d device driver. \n", _MALI_API_VERSION);
	printk("Compiled: %s, time: %s.\n", __DATE__, __TIME__);
	printk("Driver revision: %s\n", SVN_REV_STRING);

#if MALI_ENABLE_CPU_CYCLES
	mali_init_cpu_time_counters_on_all_cpus(0);
	MALI_DEBUG_PRINT(2, ("CPU cycle counter setup complete\n"));
	printk("CPU cycle counter setup complete\n");
	...
	if (0 != err) {
		...
		printk("Failed to initialize profiling, feature will be unavailable\n");
	}
#endif
