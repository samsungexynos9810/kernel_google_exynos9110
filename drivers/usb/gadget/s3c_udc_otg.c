...
static int udc_enable(struct s3c_udc *dev)
{
	...
	if (err) {
		pr_info(KERN_ERR "%s: can't clk prepare enable\n", __func__);
		return err;
	}

	err = phy_power_on(dev->phy);
	if (err) {
		pr_info(KERN_ERR "%s: can't power on phy\n", __func__);
		return err;
	}
	err = phy_init(dev->phy);
	if (err) {
		pr_info(KERN_ERR "%s: can't init phy\n", __func__);
		return err;
	}
...
static int s3c_udc_probe(struct platform_device *pdev)
{
	...
	dev->phy = devm_phy_get(&pdev->dev, "usb2-phy");
	if (IS_ERR(dev->phy)) {
		dev_err(&pdev->dev, "%s: no usb2 phy configured\n", __func__);
		return -EPROBE_DEFER;
	}
	...
	if (!res) {
		dev_err(&pdev->dev, "%s: cannot find resource\n", __func__);
		return -EINVAL;
	}

	dev->regs = devm_ioremap_resource(&pdev->dev, res);
	if (!dev->regs) {
		dev_err(&pdev->dev, "%s: cannot remap registers\n", __func__);
		return -EADDRNOTAVAIL;
	}
	udc_reinit(dev);

	/* get clk for otg */
	dev->clk = devm_clk_get(&pdev->dev, "otg");
	if (IS_ERR_OR_NULL(dev->clk)) {
		dev_err(&pdev->dev, "%s: couldn't get otg clock\n", __func__);
		return -EINVAL;
	}
	...
	if (!dev->ep0_data) {
		dev_err(&pdev->dev, "%s: can't get ep0_data dma memory\n",
			__func__);
		retval = -ENOMEM;
...
#ifdef CONFIG_PM
static int s3c_udc_suspend(struct device *tdev)
{
	struct s3c_udc *dev = the_controller;
	int i;
...
static int s3c_udc_resume(struct device *tdev)
{
	struct s3c_udc *dev = the_controller;
...
static const struct of_device_id exynos_udc_match[] = {
	{
		.compatible = "samsung,exynos3250_udc",
	},
	{},
};
