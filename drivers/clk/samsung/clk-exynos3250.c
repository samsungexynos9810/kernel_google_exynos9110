...
#define CMUX_S(_id, cname, pnames, o, s, w, so, ss, sw) \
                MUX_STAT(_id, cname, pnames, (unsigned long)o, s, w, (unsigned long)so, ss, sw)

static struct samsung_mux_clock exynos3250_mux_clks[] __initdata = {
	CMUX_S(CLK_MOUT_MPLL_USER_L, "mout_mpll_user_l", mout_mpll_user_p,
			SRC_LEFTBUS, 4, 1, MUX_STAT_LEFTBUS, 4, 3),
	CMUX_S(CLK_MOUT_GDL, "mout_gdl", mout_gdl_p, SRC_LEFTBUS, 0, 1,
			MUX_STAT_LEFTBUS, 0, 3),

	CMUX_S(CLK_MOUT_MPLL_USER_R, "mout_mpll_user_r", mout_mpll_user_r_p,
			SRC_RIGHTBUS, 4, 1, MUX_STAT_RIGHTBUS, 4, 3),
	CMUX_S(CLK_MOUT_GDR, "mout_gdr", mout_gdr_p, SRC_RIGHTBUS, 0, 1,
			MUX_STAT_RIGHTBUS, 0 ,3),

	CMUX_S(CLK_MOUT_UPLL, "mout_upll", mout_upll_p, SRC_TOP1, 28, 1,
			MUX_STAT_TOP1, 28, 3),
	CMUX_S(CLK_MOUT_ACLK_400_MCUISP_SUB, "mout_aclk_400_mcuisp_sub",
			mout_aclk_400_mcuisp_sub_p, SRC_TOP1, 24, 1,
			MUX_STAT_TOP1, 24, 3),
	CMUX_S(CLK_MOUT_ACLK_266_SUB, "mout_aclk_266_sub", mout_aclk_266_sub_p,
			SRC_TOP1, 20, 1, MUX_STAT_TOP1, 20, 3),
	CMUX_S(CLK_MOUT_MPLL, "mout_mpll", mout_mpll_p,
			SRC_TOP1, 12, 1, MUX_STAT_TOP1, 12 ,3),
	CMUX_S(CLK_MOUT_ACLK_400_MCUISP, "mout_aclk_400_mcuisp",
			mout_aclk_400_mcuisp_p, SRC_TOP1, 8, 1,
			MUX_STAT_TOP1, 8, 3),
	CMUX(CLK_MOUT_VPLLSRC, "mout_vpllsrc", mout_vpllsrc_p, SRC_TOP1, 0, 1),

	CMUX_S(CLK_MOUT_ACLK_200, "mout_aclk200", group_div_mpll_pre_p,
		SRC_TOP0, 24, 1, MUX_STAT_TOP0, 24, 3),
	CMUX_S(CLK_MOUT_ACLK_160, "mout_aclk_160", group_div_mpll_pre_p,
		SRC_TOP0, 20, 0, MUX_STAT_TOP0, 20, 3),
	CMUX_S(CLK_MOUT_ACLK_100, "mout_aclk100", group_div_mpll_pre_p,
		SRC_TOP0, 16, 1, MUX_STAT_TOP0, 16, 3),
	CMUX_S(CLK_MOUT_ACLK_266_1, "mout_aclk_266_1", mout_aclk_266_1_p,
		SRC_TOP0, 14, 1, MUX_STAT_TOP1, 4, 3),
	CMUX_S(CLK_MOUT_ACLK_266_0, "mout_aclk_266_0", mout_aclk_266_0_p,
		SRC_TOP0, 13, 1, MUX_STAT_TOP1, 0, 3),
	CMUX_S(CLK_MOUT_ACLK_266, "mout_aclk_266", mout_aclk_266_p,
		SRC_TOP0, 12, 1, MUX_STAT_TOP0, 12, 3),
	CMUX_S(CLK_MOUT_VPLL, "mout_vpll", mout_vpll_p, SRC_TOP0, 8, 1,
		MUX_STAT_TOP0, 8, 3),
	CMUX_S(CLK_MOUT_EPLL, "mout_epll", mout_epll_p, SRC_TOP0, 4, 1,
		MUX_STAT_TOP0, 4, 3),
	...
	CMUX_S(CLK_MOUT_G3D, "mout_g3d", mout_g3d_p, SRC_G3D, 8, 1,
			MUX_STAT_G3D, 8, 3),
	CMUX_S(CLK_MOUT_G3D_1, "mout_g3d_1", group_epll_vpll_p, SRC_G3D, 4, 1,
			MUX_STAT_G3D, 4, 3),
	CMUX_S(CLK_MOUT_G3D_0, "mout_g3d_0", group_div_mpll_pre_p, SRC_G3D, 0, 1,
			MUX_STAT_G3D, 0, 3),

	CMUX_S(CLK_MOUT_MFC, "mout_mfc", mout_mfc_p, SRC_MFC, 8, 1,
			MUX_STAT_MFC, 8, 3),
	CMUX_S(CLK_MOUT_MFC_1, "mout_mfc_1", group_epll_vpll_p, SRC_MFC, 4, 1,
			MUX_STAT_MFC, 4, 3),
	CMUX_S(CLK_MOUT_MFC_0, "mout_mfc_0", group_div_mpll_pre_p, SRC_MFC, 0, 1,
			MUX_STAT_MFC, 0, 3),
...
#define CDIV_F_S(_id, cname, pname, o, s, w, f, df, so, ss, sw) \
			DIV_F_STAT(_id, cname, pname, (unsigned long)o, s, w, \
			f, df, (unsigned long)so, ss, sw)
#define CDIV_S(_id, cname, pnames, o, s, w, so, ss, sw) \
                DIV_STAT(_id, cname, pnames, (unsigned long)o, s, w, \
		(unsigned long)so, ss, sw)

static struct samsung_div_clock exynos3250_div_clks[] __initdata = {
	CDIV_S(CLK_DIV_GPL, "dout_gpl", "dout_gdl", DIV_LEFTBUS, 4, 3,
		DIV_STAT_LEFTBUS, 4, 1),
	CDIV_S(CLK_DIV_GDL, "dout_gdl", "mout_gdl", DIV_LEFTBUS, 0, 4,
		DIV_STAT_LEFTBUS, 0, 1),

	CDIV_S(CLK_DIV_GPR, "dout_gpr", "dout_gdr", DIV_RIGHTBUS, 4, 3,
		DIV_STAT_RIGHTBUS, 4, 1),
	CDIV_S(CLK_DIV_GDR, "dout_gdr", "mout_gdr", DIV_RIGHTBUS, 0, 4,
		DIV_STAT_RIGHTBUS, 0, 1),

	CDIV_S(CLK_DIV_MPLL_PRE, "dout_mpll_pre", "sclk_mpll_mif", DIV_TOP,
		28, 2, DIV_STAT_TOP, 20 ,1),
	CDIV_S(CLK_DIV_ACLK_400_MCUISP, "dout_aclk_400_mcuisp",
		"mout_aclk_400_mcuisp", DIV_TOP, 24, 3, DIV_STAT_TOP, 24, 1),
	CDIV_S(CLK_DIV_ACLK_200, "dout_aclk_200", "mout_aclk200", DIV_TOP, 12, 3,
		DIV_STAT_TOP, 12, 1),
	CDIV_S(CLK_DIV_ACLK_160, "dout_aclk_160", "mout_aclk_160", DIV_TOP, 8, 3,
		DIV_STAT_TOP, 8, 1),
	CDIV_S(CLK_DIV_ACLK_100, "dout_aclk_100", "mout_aclk100", DIV_TOP, 4, 4,
		DIV_STAT_TOP, 4, 1),
	CDIV_S(CLK_DIV_ACLK_266, "dout_aclk_266", "mout_aclk_266", DIV_TOP, 0, 3,
		DIV_STAT_TOP, 0, 1),

	CDIV_F_S(CLK_DIV_TSADC_PRE, "dout_tsadc_pre", "dout_tsadc", DIV_FSYS0,
			8, 8, CLK_SET_RATE_PARENT, 0, DIV_STAT_FSYS0, 8, 1),
	CDIV_S(CLK_DIV_TSADC, "dout_tsadc", "mout_tsadc", DIV_FSYS0, 0, 4,
			DIV_STAT_FSYS0, 0, 1),
	...
	CDIV_F_S(CLK_DIV_MMC1_PRE, "dout_mmc1_pre", "dout_mmc1", DIV_FSYS1,
			24, 8, CLK_SET_RATE_PARENT, 0, DIV_STAT_FSYS1, 24, 1),
	CDIV_S(CLK_DIV_MMC1, "dout_mmc1", "mout_mmc1", DIV_FSYS1, 16, 4,
			DIV_STAT_FSYS1, 16, 1),
	CDIV_F_S(CLK_DIV_MMC0_PRE, "dout_mmc0_pre", "dout_mmc0", DIV_FSYS1,
			8, 8, CLK_SET_RATE_PARENT, 0, DIV_STAT_FSYS1, 8, 1),
	CDIV_S(CLK_DIV_MMC0, "dout_mmc0", "mout_mmc0", DIV_FSYS1, 0, 4,
			DIV_STAT_FSYS1, 0, 1),

	CDIV_S(CLK_DIV_UART3, "dout_uart3", "mout_uart3", DIV_PERIL0, 12, 4,
			DIV_STAT_PERIL0, 12, 1),
	CDIV_S(CLK_DIV_UART2, "dout_uart2", "mout_uart2", DIV_PERIL0, 8, 4,
			DIV_STAT_PERIL0, 8, 1),
	CDIV_S(CLK_DIV_UART1, "dout_uart1", "mout_uart1", DIV_PERIL0, 4, 4,
			DIV_STAT_PERIL0, 4, 1),
	CDIV_S(CLK_DIV_UART0, "dout_uart0", "mout_uart0", DIV_PERIL0, 0, 4,
			DIV_STAT_PERIL0, 0, 1),

	CDIV_S(CLK_DIV_AUDIO, "dout_audio", "mout_audio", DIV_PERIL4, 16, 4,
			DIV_STAT_PERIL4, 16, 1),

	CDIV_S(CLK_DIV_I2S, "dout_i2s", "dout_audio", DIV_PERIL5, 8, 6,
			DIV_STAT_PERIL5, 8, 1),

	CDIV_S(CLK_DIV_MIPI0_PRE, "dout_mipi0_pre", "dout_mipi0", DIV_LCD,
			20, 4, DIV_STAT_LCD, 20, 1),
	CDIV_S(CLK_DIV_MIPI0, "dout_mipi0", "mout_mipi0", DIV_LCD, 16, 4,
			DIV_STAT_LCD, 16, 1),
	CDIV_S(CLK_DIV_FIMD0, "dout_fimd0", "mout_fimd0", DIV_LCD, 0, 4,
			DIV_STAT_LCD, 0, 1),

	CDIV_S(CLK_DIV_G3D, "dout_g3d", "mout_g3d", DIV_G3D, 0, 4,
			DIV_STAT_G3D, 0, 1),

	CDIV_S(CLK_DIV_MFC, "dout_mfc", "mout_mfc", DIV_MFC, 0, 4,
			DIV_STAT_MFC, 0, 1),

	CDIV_S(CLK_DIV_CAM1, "dout_cam1", "mout_cam1", DIV_CAM, 20, 4,
			DIV_STAT_CAM, 20, 1),
	CDIV_S(CLK_DIV_CAM_BLK, "dout_cam_blk", "mout_cam_blk", DIV_CAM, 0, 4,
			DIV_STAT_CAM, 0, 1),

	CDIV_S(CLK_DIV_SPI1_PRE, "dout_spi1_pre", "dout_spi1", DIV_PERIL1,
			24, 8, DIV_STAT_PERIL1, 24, 1),
	CDIV_S(CLK_DIV_SPI1, "dout_spi1", "mout_spi1", DIV_PERIL1, 16, 4,
			DIV_STAT_PERIL1, 16, 1),
	CDIV_S(CLK_DIV_SPI0_PRE, "dout_spi0_pre", "dout_spi0", DIV_PERIL1, 8, 8,
			DIV_STAT_PERIL1, 8, 1),
	CDIV_S(CLK_DIV_SPI0, "dout_spi0", "mout_spi0", DIV_PERIL1, 0, 4,
			DIV_STAT_PERIL1, 0, 1),

	CDIV_S(CLK_DIV_UART_ISP, "dout_uart_isp", "mout_uart_isp",
			DIV_ISP, 28, 4, DIV_STAT_ISP, 28 ,1),
	CDIV_S(CLK_DIV_SPI1_ISP_PRE, "dout_spi1_isp_pre", "dout_spi1_isp",
			DIV_ISP, 20, 8, DIV_STAT_ISP, 20, 1),
	CDIV_S(CLK_DIV_SPI1_ISP, "dout_spi1_isp", "mout_spi1_isp",
			DIV_ISP, 16, 4, DIV_STAT_ISP, 16, 1),
	CDIV_S(CLK_DIV_SPI0_ISP_PRE, "dout_spi0_isp_pre", "dout_spi0_isp",
			DIV_ISP, 8, 8, DIV_STAT_ISP, 8, 1),
	CDIV_S(CLK_DIV_SPI0_ISP, "dout_spi0_isp", "mout_spi0_isp",
			DIV_ISP, 4, 4, DIV_STAT_ISP, 4, 1),

	CDIV_S(CLK_DIV_ISP1, "dout_isp1", "mout_aclk_266_sub", DIV_ISP0, 4, 3,
			DIV_STAT_ISP0, 4, 1),
	CDIV_S(CLK_DIV_ISP0, "dout_isp0", "mout_aclk_266_sub", DIV_ISP0, 0, 3,
			DIV_STAT_ISP0, 0, 1),

	CDIV_S(CLK_DIV_MCUISP1, "dout_mcuisp1", "mout_aclk_400_mcuisp_sub",
			DIV_ISP1, 8, 3, DIV_STAT_ISP1, 8, 1),
	CDIV_S(CLK_DIV_MCUISP0, "dout_mcuisp0", "mout_aclk_400_mcuisp_sub",
			DIV_ISP1, 4, 3, DIV_STAT_ISP1, 4, 1),
	CDIV_S(CLK_DIV_MPWM, "dout_mpwm", "dout_isp1", DIV_ISP1, 0, 3,
			DIV_STAT_ISP1, 0, 1),
};
