...
int fimc_is_ischain_power(struct fimc_is_device_ischain *device, int on)
{
	...
	if (on) {
	    ...
	    writel(0x00, PMUREG_ISP_ARM_OPTION);
#else
		timeout = 1000;
...
#if CONFIG_SOC_EXYNOS3250
	scc_param->dma_output.width = chain1_width;
	scc_param->dma_output.height = chain1_height;
	...
	struct fimc_is_queue *queue)
{
	...
	//device->chain0_width = device->sensor_width;
	//device->chain0_height = device->sensor_height;

	device->chain0_width = device->sensor_width - device->margin_width;
	device->chain0_height = device->sensor_height - device->margin_height;
	...
	//jmq.todo.to check the crop size.
...
#if defined(HAS_FW_CLOCK_GATE)
	ret = fimc_is_itf_sys_ctl(device, IS_SYS_CLOCK_GATE, sysfs_debug.clk_clk_gate);
	if (ret) {
	...
	u32 *indexes)
{
	...
	if (test_bit(FIMC_IS_GROUP_OTF_INPUT, &device->group_3aa.state)) {
		...
		taa_otf_input = fimc_is_itf_g_param(device, frame, PARAM_3AA_OTF_INPUT);
		taa_otf_input->bds_out_enable = ISP_BDS_COMMAND_DISABLE;
...
{
	int ret = 0;
	struct param_dma_output *taa_vdma_output;
...
#ifdef CONFIG_SOC_EXYNOS3250
	/*VDMA5*/
	taa_vdma_output = fimc_is_itf_g_param(device, frame, PARAM_ISP_VDMA5_OUTPUT);
	taa_vdma_output->cmd = DMA_INPUT_COMMAND_ENABLE;
	taa_vdma_output->width = output_crop[2];
	taa_vdma_output->height = output_crop[3];
	taa_vdma_output->buffer_number = 0;
	taa_vdma_output->buffer_address = 0;
	taa_vdma_output->dma_out_mask = 0;
	taa_vdma_output->bitwidth = DMA_OUTPUT_BIT_WIDTH_8BIT;
	taa_vdma_output->notify_dma_done = DMA_OUTPUT_NOTIFY_DMA_DONE_ENBABLE;


	switch (queue->framecfg.format.pixelformat) {
	case V4L2_PIX_FMT_YUYV:
		taa_vdma_output->format = DMA_OUTPUT_FORMAT_YUV422,
		taa_vdma_output->plane = DMA_OUTPUT_PLANE_1;
		taa_vdma_output->order = DMA_OUTPUT_ORDER_CrYCbY;
		break;
	case V4L2_PIX_FMT_NV21M:
	case V4L2_PIX_FMT_NV21:
		taa_vdma_output->format = OTF_OUTPUT_FORMAT_YUV420,
		taa_vdma_output->plane = DMA_OUTPUT_PLANE_2;
		taa_vdma_output->order = DMA_OUTPUT_ORDER_CbCr;
		break;
	case V4L2_PIX_FMT_NV12M:
	case V4L2_PIX_FMT_NV12:
		taa_vdma_output->format = OTF_OUTPUT_FORMAT_YUV420,
		taa_vdma_output->plane = DMA_OUTPUT_PLANE_2;
		taa_vdma_output->order = DMA_OUTPUT_ORDER_CrCb;
		break;
	case V4L2_PIX_FMT_YUV420M:
	case V4L2_PIX_FMT_YVU420M:
		taa_vdma_output->format = OTF_OUTPUT_FORMAT_YUV420,
		taa_vdma_output->plane = DMA_OUTPUT_PLANE_3;
		taa_vdma_output->order = DMA_OUTPUT_ORDER_NO;
		break;
	default:
		mwarn("unknown ISP YUV output pixelformat:0x%x", device,queue->framecfg.format.pixelformat);
		break;
	}
	...
	*hindex |= HIGHBIT_OF(PARAM_ISP_VDMA5_OUTPUT);
	(*indexes)++;
#else
	taa_vdma_output = fimc_is_itf_g_param(device, frame, PARAM_3AA_VDMA4_OUTPUT);
	taa_vdma_output->cmd = DMA_OUTPUT_COMMAND_ENABLE;
	taa_vdma_output->width = output_crop[2];
	taa_vdma_output->height = output_crop[3];
	taa_vdma_output->buffer_number = 0;
	taa_vdma_output->buffer_address = 0;
	taa_vdma_output->dma_out_mask = 0;
	taa_vdma_output->bitwidth = DMA_OUTPUT_BIT_WIDTH_12BIT;
	taa_vdma_output->notify_dma_done = DMA_OUTPUT_NOTIFY_DMA_DONE_ENBABLE;

	if (queue->framecfg.format.pixelformat == V4L2_PIX_FMT_SBGGR12) {
		taa_vdma_output->format = DMA_INPUT_FORMAT_BAYER_PACKED12;
	} else if (queue->framecfg.format.pixelformat == V4L2_PIX_FMT_SBGGR16) {
		taa_vdma_output->format = DMA_INPUT_FORMAT_BAYER;
	} else {
		mwarn("Invalid bayer format", device);
		ret = -EINVAL;
		goto p_err;
...
#ifdef CONFIG_SOC_EXYNOS3250
	taa_vdma_output = fimc_is_itf_g_param(device, frame, PARAM_ISP_VDMA5_OUTPUT);
	taa_vdma_output->cmd = DMA_OUTPUT_COMMAND_DISABLE;
	*lindex |= LOWBIT_OF(PARAM_ISP_VDMA5_OUTPUT);
	*hindex |= HIGHBIT_OF(PARAM_ISP_VDMA5_OUTPUT);
	(*indexes)++;
#else
...
#ifdef CONFIG_SOC_EXYNOS3250
int fimc_is_ischain_scc_s_format(struct fimc_is_device_ischain *device,
	u32 pixelformat, u32 width, u32 height)
{
	...
	//jmq.todo.we need to get the chain resolution from 3aa0/scc video node
	...
	struct fimc_is_frame *check_frame)
{
	...
	for (capture_id = 0; capture_id < CAPTURE_NODE_MAX; ++capture_id) {
		...
		case 0:
			break;
		case FIMC_IS_VIDEO_3A0_NUM:
		case FIMC_IS_VIDEO_3A1_NUM:
