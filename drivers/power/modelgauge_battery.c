...
static int nermy_regmap_read(struct regmap *map, unsigned int reg, unsigned int *val)
{
	int ret;
...
static void nermy_regmap_write(struct regmap *regmap, unsigned int addr, unsigned int reg)
{
	...
	u16 *data)
{
	...
	/* RAM has different endianness than registers */
