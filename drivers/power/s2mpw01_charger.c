		...
		int onoff)
{
	...
	if (charger->dev_id < EVT_3) {
		...
		pr_debug("[DEBUG]%s: onoff[%d], chg_en_data[0x%x], acok[0x%x]\n",
	...
	if (onoff > 0) {
		...
		if (!(acok_stat & ACOK_STATUS_MASK) && (charger->dev_id < EVT_3)) {
			...
			s2mpw01_write_reg(charger->client, 0x2E, data);
			s2mpw01_update_reg(charger->client, S2MPW01_CHG_REG_CTRL1, EN_CHG_MASK, EN_CHG_MASK);
			pr_debug("[DEBUG] %s: turn on charger for RID detection\n", __func__);
		}
	}
}
...
static void s2mpw01_topoff_interrupt_onoff(struct s2mpw01_charger_data *charger, int onoff)
{
	...
	if (onoff > 0) {
		...
		pr_debug("[DEBUG]%s: Use top-off interrupt: 0x%x, 0x%x\n", __func__,
		...
		pr_debug("[DEBUG]%s: Top-off interrupt Masking: 0x%x, 0x%x\n", __func__,
			charger->iodev->irq_masks_cur[3], charger->iodev->irq_masks_cache[3]);
		charger->iodev->irq_masks_cur[3] |= 0x04;
		charger->iodev->irq_masks_cache[3] |= 0x04;
		...
		int charging_current)
{
	...
	pr_debug("[DEBUG]%s: fast charge current  %d\n", __func__, charging_current);
...
static int s2mpw01_get_current_eoc_setting(struct s2mpw01_charger_data *charger)
{
	...
	pr_debug("[DEBUG]%s: data(0x%x), top-off current	%d\n", __func__, data, eoc_current[data]);
...
static void s2mpw01_set_topoff_current(struct i2c_client *i2c, int current_limit)
{
	...
	pr_debug("[DEBUG]%s: top-off current	%d, data=0x%x\n", __func__, current_limit, data);
...
static void s2mpw01_configure_charger(struct s2mpw01_charger_data *charger)
{
	...
	dev_dbg(dev, "%s() set configure charger\n", __func__);

	if (charger->charging_current < 0) {
		dev_dbg(dev, "%s() OTG is activated. Ignore command!\n",
				__func__);
		return;
	}

	if (!charger->pdata->charging_current_table) {
		dev_dbg(dev, "%s() table is not exist\n", __func__);
		return;
	}
	...
	dev_dbg(dev, "%s() fast charging current (%dmA)\n",
...
{
	dev_dbg(&charger->client->dev, "%s : DEV ID : 0x%x\n", __func__,
			charger->dev_id);
...
static int s2mpw01_get_charging_health(struct s2mpw01_charger_data *charger)
{
	...
	if ((data1 & ACOK_STATUS_MASK) && (data & CHGVINOVP_STATUS_MASK) &&
		...
		pr_debug("[%s] POWER_SUPPLY_HEALTH_OVERVOLTAGE, unhealth_cnt %d\n ",
...
/*	int previous_cable_type = charger->cable_type; */
	...
	case POWER_SUPPLY_PROP_ONLINE:
		...
		if (charger->cable_type == POWER_SUPPLY_TYPE_BATTERY ||
			...
			dev_dbg(dev, "%s() [BATT] Type Battery\n", __func__);
			if (!charger->pdata->charging_current_table)
				return -EINVAL;
			...
			dev_dbg(dev, "%s() OTG mode not supported\n", __func__);
		} else {
			dev_dbg(dev, "%s()  Set charging, Cable type = %d\n",
	...
	case POWER_SUPPLY_PROP_POWER_NOW:
		...
		dev_dbg(dev, "%s() Set Power Now -> chg current = %d mA, eoc = %d mA\n",
...
static irqreturn_t s2mpw01_tmrout_isr(int irq, void *data)
{
	...
	if (val & 0x10) {
		/* Timer out status */
		pr_debug("%s, fast-charging timeout, timer clear\n", __func__);
		s2mpw01_enable_charger_switch(charger, 0);
		msleep(100);
		s2mpw01_enable_charger_switch(charger, 1);
	}
		...
		struct s2mpw01_charger_platform_data *pdata)
{
	...
	dev_dbg(dev, "s2mpw01 charger parse dt retval = %d\n", ret);
	return ret;
}
#else
static int s2mpw01_charger_parse_dt(struct device *dev,
...
static int s2mpw01_charger_probe(struct platform_device *pdev)
{
	...
	pr_debug("%s:[BATT] S2MPW01 Charger driver probe\n", __func__);
	...
	if (ret < 0) {
		dev_dbg(s2mpw01->dev, "%s: Fail to request charger irq in IRQ: %d: %d\n",
	...
	if (ret < 0) {
		pr_debug("%s: failed to read PM addr 0x41(%d)\n", __func__, ret);
		goto err_power_supply_register;
	}
	...
	if (ret < 0)
		pr_debug("{DEBUG] %s : chg status2 read fail\n", __func__);

	ret = s2mpw01_read_reg(charger->client, S2MPW01_CHG_REG_STATUS3, &chg_sts3);
	if (ret < 0)
		pr_debug("{DEBUG] %s : chg status3 read fail\n", __func__);
	...
	if (ret < 0) {
		pr_debug("%s: failed to read S2MPW01_PMIC_REG_STATUS1(%d)\n", __func__, ret);
		goto err_power_supply_register;
	}
	...
	pr_debug("%s:[BATT] S2MPW01 charger driver loaded OK\n", __func__);
...
static int s2mpw01_charger_resume(struct device *dev)
{
	...
	pr_debug("%s : CHG_INT1 ---> 0x%x\n", __func__, val);
	pr_debug("[DEBUG]%s: Top-off interrupt Masking : 0x%x, topoff status %d\n", __func__,
...
static void s2mpw01_charger_shutdown(struct device *dev)
{
	...
	pr_debug("%s: S2MPW01 Charger driver shutdown\n", __func__);
}
