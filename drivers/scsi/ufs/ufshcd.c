...
{
	if (hba->quirks & UFSHCD_QUIRK_GET_UPMCRS_DIRECT)
		return ufshcd_vops_get_upmcrs(hba) & 0x7;
	else
	...
	if (sg_segments) {
		...
		scsi_for_each_sg(cmd, sg, sg_segments, i) {
...
#elif defined(CONFIG_UFS_FMP_EXT4CRYPT_FS)
			if (!((unsigned long)(sg_page(sg)->mapping) & 0x1)) {
				if ((unsigned int)(sg_page(sg)->index) >= 2 &&
						((uint64_t)(sg_page(sg)->mapping)) &&
						((unsigned int)(sg_page(sg)->mapping->use_fmp)))
					sector_key |= UFS_FILE_ENCRYPTION_SECTOR_BEGIN;
				else
					sector_key &= ~UFS_FILE_ENCRYPTION_SECTOR_BEGIN;
			} else
			...
			if (sector_key == UFS_BYPASS_SECTOR_BEGIN) {
				...
				unsigned long flags;
				ret = fmp_ufs_map_sg(prd_table, sg, sector_key, i, sector, cmd->request->bio);
				if (ret) {
