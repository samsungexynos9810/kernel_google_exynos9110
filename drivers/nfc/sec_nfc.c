...
int sec_nfc_i2c_probe(struct i2c_client *client)
{
	...
	/* jia */
	info->base_syscon = ioremap(EXYNOS3_PA_SYSCON, SZ_4K);
	//reg = __raw_readl(info->base_syscon + 0x0228);
	//reg &= ~(0x1<<0);
	__raw_writel(0x0, info->base_syscon + 0x0228);
	/* jia */
	...
	struct sec_nfc_platform_data *pdata)
{
	...
	pdata->pwr = of_get_named_gpio(np, "sec-nfc,pwr-gpio", 0); // jia
#ifdef CONFIG_SEC_NFC_IF_I2C
...
#endif
	/* jia */
	ret = gpio_request(pdata->pwr, "nfc_pwr");

	if (ret) {
		dev_err(dev, "failed to get gpio ven\n");
		goto err_gpio_pwr;
	}
	gpio_direction_output(pdata->pwr, 1);
	/* jia */
