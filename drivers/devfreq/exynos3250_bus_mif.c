...
static int exynos3250_devfreq_set_dll_voltage(struct devfreq_data_mif *data,
...
static int exynos3250_devfreq_mif_suspend(struct device *dev)
{
	...
	target_idx = exynos3250_devfreq_mif_get_idx(devfreq_mif_opp_list,
						ARRAY_SIZE(devfreq_mif_opp_list),
						exynos3250_mif_devfreq_profile.initial_freq);
	tmp = devfreq_mif_opp_list[target_idx].volt;
	regulator_set_voltage(data->vdd_mif, tmp, tmp + MIF_COLD_OFFSET);
