...
#define DEVFREQ_DISP_REBOOT_FREQ	(208000)

static struct exynos_devfreq_data *disp_data;

static int exynos7570_devfreq_disp_cmu_dump(struct device *dev,
...
static int exynos7570_devfreq_disp_reboot(struct device *dev,
					struct exynos_devfreq_data *data)
{
...
static u32 exynos7570_devfreq_disp_get_target_freq(char *name, u32 freq)
{
	return cal_dfs_get_rate_by_member(dvfs_disp, name, freq);
}

static int exynos7570_devfreq_disp_get_freq(struct device *dev, u32 *cur_freq,
				struct exynos_devfreq_data *data)
{
...
static int exynos7570_devfreq_disp_set_freq(struct device *dev,
...
static int exynos7570_devfreq_disp_init_freq_table(struct device *dev,
					struct exynos_devfreq_data *data)
{
...
static int exynos7570_devfreq_disp_get_volt_table(struct device *dev,
...
static int exynos7570_devfreq_disp_init(struct device *dev,
				struct exynos_devfreq_data *data)
{
...
static int exynos7570_devfreq_disp_exit(struct device *dev,
				struct exynos_devfreq_data *data)
{
	clk_put(data->clk);
...
static int __init exynos7570_devfreq_disp_init_prepare(struct exynos_devfreq_data *data)
{
	...
	data->ops.get_target_freq = exynos7570_devfreq_disp_get_target_freq;
