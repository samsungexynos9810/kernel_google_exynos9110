...
extern void __exynos_ss_work(struct worker *worker,
...
static inline void exynos_ss_work(struct worker *worker,
				      struct work_struct *work, work_func_t f)
{
	__exynos_ss_work(worker, work, f);
}
...
#define exynos_ss_work(a,b,c)		do { } while(0)
