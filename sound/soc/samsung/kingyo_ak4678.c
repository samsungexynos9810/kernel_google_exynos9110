 ...
 */
static ssize_t mixer_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (argc < 2 || kstrtouint(argv[1], 16, &data1)
		|| (argc > 2 && kstrtouint(argv[2], 16, &data2))) 	return count;

	switch (*argv[0]) {
		case 'a':
			dev_dbg(dev, "iomap 0x%x\n", data1);
			if (basepaddr != data1) {
				if (basevaddr != NULL) iounmap(basevaddr);
				basepaddr = data1;
...
static ssize_t mixer_show(struct device *dev,
...
static DEVICE_ATTR(mixer, S_IRUGO | S_IWUSR | S_IWGRP,
		mixer_show, mixer_store);
