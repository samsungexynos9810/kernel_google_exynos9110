...
/*
 * tai hw params. (AP I2S Master with mic)
 */

static int tai_hw_params(
...
static int tai_aif_startup(struct snd_pcm_substream *substream)
{
	gprintk("\n");
	return 0;
}

static void tai_aif_shutdown(struct snd_pcm_substream *substream)
{
	gprintk("\n");
}
#endif

static struct snd_soc_ops tai_ops = {
#ifdef AUDIOIF_POWER_SUPPLY
	.startup = tai_aif_startup,
	.shutdown = tai_aif_shutdown,
#endif
	.hw_params = tai_hw_params,
};

static int tai_ak4678_init_paiftx(struct snd_soc_pcm_runtime *rtd)
{
#if 0
...
static struct snd_soc_dai_link tai_dai[] = {
	{
		.name = "Tai_AK4678_Mic",
		.stream_name = "Capture",
		.codec_name = "ak4678mic.3-0012",
		.codec_dai_name = "ak4678-mic",
		.init = tai_ak4678_init_paiftx,
...
static int tai_probe(struct snd_soc_card *card)
{
	gprintk("\n");
...
static int tai_remove(struct snd_soc_card *card)
{
	gprintk("\n");
	return 0;
}

static struct snd_soc_card tai_card = {
	.name = "TAI-AK4678",
	.probe = tai_probe,
	.remove = tai_remove,
	.dai_link = tai_dai,
	.num_links = ARRAY_SIZE(tai_dai),
};
...
static int tai_ak4678_probe(struct platform_device *pdev)
{
	int ret;
	struct device_node *np = pdev->dev.of_node;
	struct snd_soc_card *card = &tai_card;
...
static int tai_ak4678_remove(struct platform_device *pdev)
{
...
static const struct of_device_id akm_ak4678_of_match[] = {
	{
		.compatible = "samsung,tai-ak4678",
	},
	{},
};
MODULE_DEVICE_TABLE(of, akm_ak4678_of_match);

static struct platform_driver tai_ak4678_driver = {
	.driver =
		{
			.name = "TAI-AK4678",
			.owner = THIS_MODULE,
	...
	.probe = tai_ak4678_probe,
	.remove = tai_ak4678_remove,
};
module_platform_driver(tai_ak4678_driver);

MODULE_DESCRIPTION("AK4678 ALSA SoC Driver for Tai Board");
MODULE_LICENSE("GPL");
